...
string priv_tx_ref_block_num_or_id;
...
eosio::priv_chain_apis::read_only::get_info_results get_priv_info(){
   return call(url, get_priv_info_func).as<eosio::priv_chain_apis::read_only::get_info_results>();
}
...
fc::variant determine_required_keys(const signed_transaction& trx) {
   ...
   //std::cout << "wook : cleos : determine_required_keys() : public keys : " << fc::variant(public_keys).as_string() << std::endl;
   auto get_arg = fc::mutable_variant_object
           ("transaction", (transaction)trx)
           ("available_keys", public_keys);
   const auto& required_keys = call(get_required_keys, get_arg);
   //std::cout << "wook : cleos : determine_required_keys() : required_keys : " << fc::json::to_string(required_keys)<< std::endl;
...
fc::variant push_transaction( signed_transaction& trx, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   { // #5445 can't change txn content if already signed
      ...
      if (!tx_skip_sign) {
         ...
         ///std::cout << "           E O S I O           :    [START 2-1] [Main] [push_transaction] [trx] " << fc::json::to_string(required_keys) << std::endl;
         //std::cout << "           E O S I O           :    [START 2-1] [Main] [push_transaction] [trx ispriv] " << trx.__priv() << std::endl;

         sign_transaction(trx, required_keys, info.chain_id);
      }
      //std::cout << "           E O S I O           :    [START 3] [Main] [push_transaction] [trx.priv()] " << trx.__priv() << std::endl;
      ...
      trx.expiration = info2.head_block_time + tx_expiration;
      block_id_type priv_ref_block_id = info2.last_irreversible_block_id;
      try{
         fc::variant priv_ref_block;
         if (!priv_tx_ref_block_num_or_id.empty())
         {
            priv_ref_block = call(get_priv_block_func, fc::mutable_variant_object("block_num_or_id", priv_tx_ref_block_num_or_id));
            priv_ref_block_id = priv_ref_block["id"].as<block_id_type>();
         }
      }EOS_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", priv_tx_ref_block_num_or_id));
      if (trx.__priv())
      {
         trx.set_reference_block(priv_ref_block_id);
      }
      trx.max_cpu_usage_ms = tx_max_cpu_usage;
      trx.max_net_usage_words = (tx_max_net_usage + 7) / 8;
      trx.delay_sec = delaysec;
      ...
      if (!tx_skip_sign) {
         auto required_keys = determine_required_keys(trx);
         sign_transaction(trx, required_keys, info2.chain_id);
      }
      if (!tx_dont_broadcast) {
         return call(priv_push_txn_func, packed_transaction(trx, compression));
      } else {
         if (!tx_return_packed) {
            return fc::variant(trx);
         } else {
...
fc::variant priv_push_actions(std::vector<chain::action>&& actions, int32_t extra_kcpu, packed_transaction::compression_type compression = packed_transaction::none ) {
   signed_transaction trx;
   trx.actions = std::forward<decltype(actions)>(actions);
   trx.isPriv = true;
   fc::variant ret = push_transaction(trx, extra_kcpu, compression);
   return ret;
}
void print_action_tree( const fc::variant& action );
void print_result( const fc::variant& result );

void priv_print_result(const fc::variant &result)
{
   try
   {
      if (result.is_object() && result.get_object().contains("processed"))
      {
         ...
         //cout << "WOOK : print result : " << processed["block_num"].as_string() << endl;

         cerr << status << " priv transaction: " << transaction_id << "  ";
         if (net < 0)
         {
            cerr << "<unknown>";
         }
...
fc::variant push_actions(std::vector<chain::action>&& actions, int32_t extra_kcpu, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   if (tx_print_json)
   {
      cout << fc::json::to_pretty_string(ret) << endl;
   }
   else
   {
      print_result(ret);
   ...
   for(auto& a : trx.actions){
      //std::cout << "           E O S I O           :    [...] [Main] [push_actions] [trx.action] " << a.name << std::endl;
      if( a.name == newaccount)
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      else if( a.name == setcode){
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      else if ( a.name == issue)
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      else if ( a.name == create)
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      else if ( a.name == "setpriv")
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      ...
      else if ( a.name == "upsert")
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      else if ( a.name == "erase")
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      else if ( a.name == "transfer")
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      else if ( a.name == "voteproducer")
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      else if ( a.name == "regproducer")
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
      else if (a.name == "delegatebw")
      {
         trx.isPriv = true;
         ret = push_transaction(trx, extra_kcpu, compression);
      }
...
using std::cout;
void send_actions(std::vector<chain::action>&& actions, bool flag = false, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   auto result = push_actions(move(actions), extra_kcpu, compression);
   // if(flag != true)
   // {
   //    std::cout << "send_actions : 111" << std::endl;
   //    result = push_actions(move(actions), extra_kcpu, compression);
   // }
   // else
   // {
   //    std::cout << "send_actions : 222" << std::endl;
   //    result = priv_push_actions(move(actions), extra_kcpu, compression);
   // }

   if( tx_print_json ) {
      cout << fc::json::to_pretty_string( result ) << endl;
   } else {
      priv_print_result( result );
   }
}
...
struct priv_create_account_subcommand
{
   string creator;
   string account_name;
   ...
   priv_create_account_subcommand(CLI::App *actionRoot, bool s) : simple(s)
   {
      auto createAccount = actionRoot->add_subcommand(
          (simple ? "privAccount" : "privNewaccount"),
          (simple ? localized("Create a new priv account on the blockchain (assumes system contract does not restrict RAM usage)")
                  : localized("Create a new priv_account on the blockchain with initial resources")));
      ...
      createAccount->set_callback([this] {
         ...
         if (!simple)
         {
            ...
            if (net.get_amount() != 0 || cpu.get_amount() != 0)
            {
               ...
               send_actions({create, buyram, delegate}, true);
            }
            else
            {
               send_actions({create, buyram}, true);
            }
         }
         else
         {
            send_actions({create}, true);
         }
      });
   }
};
...
void priv_get_account( const string& accountName, const string& coresym, bool json_format ) {
   fc::variant json;
   if (coresym.empty()) {
      json = call(get_priv_account_func, fc::mutable_variant_object("account_name", accountName));
   }
   else {
      json = call(get_priv_account_func, fc::mutable_variant_object("account_name", accountName)("expected_core_symbol", symbol::from_string(coresym)));
   }
...
int main( int argc, char** argv ) {
   ...
   // create pirv account
   //auto privCreateAccount = priv_create_account_subcommand( create, true );
   ...
   get->add_subcommand("info", localized("Get current blockchain information"))->set_callback([] {
      ...
      std::cout << fc::json::to_pretty_string(get_priv_info()) << std::endl;
   });
   ...
   getBlock->set_callback([&blockArg,&get_bhs] {
      ...
      if( get_bhs ) {
         ...
         //std::cout << fc::json::to_pretty_string(call(get_priv_block_func, arg)) << std::endl;
      }
   });
   ...
   string privBlockArg;
   bool privGet_bhs = false;
   auto privGetBlock = get->add_subcommand("privBlock", localized("Retrieve a full block from the blockchain"), false);
   privGetBlock->add_option("privBlock", privBlockArg, localized("The number or ID of the block to retrieve"))->required();
   privGetBlock->add_flag("--header-state", privGet_bhs, localized("Get block header state from fork database instead"));
   privGetBlock->set_callback([&privBlockArg, &privGet_bhs] {
      auto arg = fc::mutable_variant_object("block_num_or_id", privBlockArg);
      if (privGet_bhs)
      {
         std::cout << fc::json::to_pretty_string(call(get_priv_block_header_state_func, arg)) << std::endl;
      }
      else
      {
         std::cout << fc::json::to_pretty_string(call(get_priv_block_func, arg)) << std::endl;
   ...
   string priv_accountName;
   string priv_coresym;
   bool priv_print_json;
   auto priv_getAccount = get->add_subcommand("priv_account", localized("Retrieve an account from the blockchain"), false);
   priv_getAccount->add_option("name", priv_accountName, localized("The name of the account to retrieve"))->required();
   priv_getAccount->add_option("core-symbol", priv_print_json, localized("The expected core symbol of the chain you are querying"));
   priv_getAccount->add_flag("--json,-j", print_json, localized("Output in JSON format"));
   priv_getAccount->set_callback([&]() { priv_get_account(priv_accountName, priv_coresym, priv_print_json); });
   ...
   getCode->set_callback([&] {
      ...
      string code_hash_priv, wasm_pr, wast_pr, abi_pr; 
      try {
         ...
         //std::cout << "main . code_bytes size : " << wasm_v.size() << std::endl;
         ...
         // priv get code
         const auto result_pr = call(get_priv_raw_code_and_abi_func, fc::mutable_variant_object("account_name", accountName));
         const std::vector<char> wasm_v_pr = result_pr["wasm"].as_blob().data;
         const std::vector<char> abi_v_pr = result_pr["abi"].as_blob().data;

         fc::sha256 hash_pr;
         if(wasm_v.size())
            hash_pr = fc::sha256::hash(wasm_v_pr.data(), wasm_v_pr.size());
         code_hash_priv = (string)hash_pr;
         //std::cout << "main . code_bytes size : " << wasm_v_pr.size() << std::endl;

         wasm_pr = string(wasm_v_pr.begin(), wasm_v_pr.end());
         if(!code_as_wasm && wasm_v_pr.size())
            wast_pr = wasm_to_wast((const uint8_t*)wasm_v_pr.data(), wasm_v_pr.size(), false);

         abi_def abi_d_pr;
         if(abi_serializer::to_abi(abi_v_pr, abi_d_pr))
            abi = fc::json::to_pretty_string(abi_d_pr);         
      }
      ...
      std::cout << localized("priv code hash: ${code_hash}", ("code_hash", code_hash_priv)) << std::endl;
   ...
   getTable->set_callback([&] {
      ...
      auto priv_result = call(get_priv_table_func, fc::mutable_variant_object("json", !binary)
                         ("code",code)
                         ("scope",scope)
                         ("table",table)
                         ("table_key",table_key) // not used
      ...
      std::cout << "Public Table Data\n"<< fc::json::to_pretty_string(result)
                << std::endl;
      // private block data table print!!
      std::cout << "Private Table Data\n" << fc::json::to_pretty_string(priv_result) << std::endl;
   });
...
//                                ->check(CLI::ExistingFile);
   ...
   // auto priv_contractSubcommand = setSubcommand->add_subcommand("priv_contract", localized("Create or update the contract on an account"));
   // priv_contractSubcommand->add_option("account", account, localized("The account to publish a contract for"))
   //                   ->required();
   // priv_contractSubcommand->add_option("contract-dir", contractPath, localized("The path containing the .wasm and .abi"));
   //                   // ->required();
   // priv_contractSubcommand->add_option("wasm-file", wasmPath, localized("The file containing the contract WASM relative to contract-dir"));
   ...
   auto set_code_callback = [&]() {
      ...
      std::cout << "main . code_bytes size : " << code_bytes.size() << std::endl;
   ...
   // add_standard_transaction_options(priv_contractSubcommand, "account@active");
   ...
   // priv_contractSubcommand->set_callback([&] {
   //    if(!contract_clear) EOS_ASSERT( !contractPath.empty(), contract_exception, " contract-dir is null ", ("f", contractPath) );
   //    shouldSend = false;
   //    set_code_callback();
   //    set_abi_callback();
