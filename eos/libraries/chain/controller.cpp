...
struct controller_impl {
   ...
   //chainbase::database            priv_db;
   //chainbase::database            priv_reversible_blocks;
   block_log                      blog;
   //block_log                      priv_blog;
   optional<pending_state>        pending;
   block_state_ptr                head;
   fork_database                  fork_db;
   //fork_database                  priv_fork_db;
   ...
   :self(s),
    ...
    /*
    priv_db(cfg.priv_state_dir,
        cfg.read_only ? database::read_only : database::read_write,
        cfg.state_size ),
    priv_reversible_blocks( cfg.priv_blocks_dir/config::priv_reversible_blocks_dir_name,
        cfg.read_only ? database::read_only : database::read_write,
        cfg.reversible_cache_size ),
    priv_blog( cfg.priv_blocks_dir ),
    priv_fork_db( cfg.priv_state_dir )
    */
   {
...
*/
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      // {
      //    std::cout << "wook : commit_block()" << fc::json::to_pretty_string(a->packed_trx->get_signed_transaction()) << std::endl;
      // }
   ...
   { try {
      ...
      if( gtrx.expiration < self.pending_block_time() ) {
         ...
         //std::cout << "controller::push_scheduled_transaction expire TRX emit(accepted_transaction,applied_transaction)\n";
      ...
      try {
         ...
         fc::move_append( pending->_actions, move(trx_context.executed) );
         //std::cout << "controller::push_scheduled_transaction try->trace->receipt = push_receipt emit(accepted_transaction,applied_transaction)\n";
         emit( self.accepted_transaction, trx );
         emit( self.applied_transaction, trace );
      ...
      if( gtrx.sender != account_name() && !failure_is_subjective(*trace->except)) {
         ...
         trace = error_trace;
         if( !trace->except_ptr ) {
            //std::cout << "controller::push_scheduled_transaction failed TRX emit(accepted_transaction,applied_transaction)\n";
            emit( self.accepted_transaction, trx );
            emit( self.applied_transaction, trace );
            undo_session.squash();
            return trace;
      ...
      if ( !subjective ) {
         ...
         //std::cout << "controller::push_scheduled_transaction Hard failed TRX emit(accepted_transaction,applied_transaction)\n";
         emit( self.accepted_transaction, trx );
         emit( self.applied_transaction, trace );

         undo_session.squash();
      } else {
         //std::cout << "controller::push_scheduled_transaction TRX emit(accepted_transaction,applied_transaction)\n";
         emit( self.accepted_transaction, trx );
         emit( self.applied_transaction, trace );
      }
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      r.status               = status;
      std::cout << "controller : push_receipt() : " << fc::json::to_pretty_string(r) << std::endl;
   ...
   as_number(const Enumeration value)
   {
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
      ...
      try {
         ...
         transaction_context trx_context(self, trn, trx->id, start); // insert the start time
         std::cout << "           E O S I O           :    [Start] [controller] [push_transaction] [isPriv] : " << trn.isPriv << std::endl;
         std::cout << "           E O S I O           :    [Start] [controller] [push_transaction] [trx ID ] : " << variant(trn.id()).as_string() << std::endl;
         ...
         try {
            ...
            if (!trx->accepted) {
               trx->accepted = true;
               std::cout << "[      controller   check point   111    ]" << std::endl;
            ...
            std::cout << "[      controller   check point   222    ]" << std::endl;
         ...
         emit( self.accepted_transaction, trx );   // traces go into state_history_plugin
         emit( self.applied_transaction, trace );
         std::cout << "           E O S I O           :    [ END ] [controller] [push_transaction]" << std::endl;
                     ...
                     const optional<block_id_type>& producer_block_id )
   {
      ...
      auto guard_pending = fc::make_scoped_exit([this](){
         pending.reset();        // what is that?
      });
      std::cout << "producer_block_id : "  << fc::variant(producer_block_id).as_string() << std::endl;
          // pending is stored in db.
      ...
      // block status is incomplete when shchedule_production_mode
      if ( read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete ) {
   ...
   void sign_block( const std::function<signature_type( const digest_type& )>& signer_callback  ) {
      //std::cerr<< " [   SIGN_BLOCK    ] " << std::endl;
      auto p = pending->_pending_block_state;

      p->sign( signer_callback );
      
      static_cast<signed_block_header&>(*p->block) = p->header;
      //std::cerr << "sign_block_header" << std::endl;
   } /// sign_block

   void apply_block( const signed_block_ptr& b, controller::block_status s ) { try {
      try {
         //std::cout << "on incoming block : apply block\n";
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>()) {
               ...
               //stored transaction in block move to packed_transactions
               //std::cout << "on_incoming_block : apply_block : packed_transactions.emplace_back \n";
         ...
         //std::cout << "on incoming block : commit block\n";
         commit_block(false);
         return;
      } catch ( const fc::exception& e ) {
         edump((e.to_detail_string()));
   ...
   std::future<block_state_ptr> create_block_state_future( const signed_block_ptr& b ) {
      ...
      auto id = b->id();
      //std::cout << "create_block_state_future : " << id << std::endl;
      // no reason for a block_state if fork_db already knows about block
      auto existing = fork_db.get_block( id ); // --> find already the id value
      //std::cerr<< "[Get_channel] Get Block ID : " << id << std::endl;
   ...
   void push_block( std::future<block_state_ptr>& block_state_future ) {
      std::cout << "[ controller.cpp ][ push_block ][ START ]" << std::endl;
      ...
      try {
         ...
         auto& b = new_header_state->block; // block : signed_block_ptr
         ...
         emit( self.accepted_block_header, new_header_state );  // what is a difference between b and new_header_state?

         if ( read_mode != db_read_mode::IRREVERSIBLE ) {
            maybe_switch_forks( s );
         }
      std::cout << "[ controller.cpp ][ push_block ][ END ]" << std::endl;
      } FC_LOG_AND_RETHROW( )
   }
   ...
   void abort_block() {
      //std::cerr << "ABORT BLOCK & RESET" << std::endl;
      if( pending ) {
         if ( read_mode == db_read_mode::SPECULATIVE ) {
   ...
   void set_trx_merkle() {
      ...
      //std::cerr << "trx_mroot : " << merkle( move(trx_digests) ) << std::endl;
   }
   ...
   // Stroing block number hash value and block summary object in id value
...
void controller::start_block( block_timestamp_type when, uint16_t confirm_block_count) {
   //std::cerr << " start_block" << std::endl;
   validate_db_available_size();
   my->start_block(when, confirm_block_count, block_status::incomplete, optional<block_id_type>() );
}
/* void controller::priv_start_block( block_timestamp_type when, uint16_t confirm_block_count) {
   validate_db_available_size();
   my->priv_start_block(when, confirm_block_count, block_status::incomplete, optional<block_id_type>() );
}
*/
...
std::future<block_state_ptr> controller::create_block_state_future( const signed_block_ptr& b ) {
   //std::cerr << "CREATE BLOCK STATE FUTURE" << std::endl;
...
optional<block_id_type> controller::pending_producer_block_id()const {
   EOS_ASSERT( my->pending, block_validate_exception, "no pending block" );
   //std::cout << "block ID of transaction data" << &my->pending->_producer_block_id << std::endl;
...
const global_property_object& controller::get_global_properties()const {
   //std::cerr << "====    (init->TRX Async) GET GLOBAL PROPERTIES    ====" <<std::endl;
   //using DB
...
void controller::validate_tapos( const transaction& trx )const { try {
   const auto& tapos_block_summary = db().get<block_summary_object>((uint16_t)trx.ref_block_num);
   std::cout << "controller [validate_tapos] [trx.block_num] : " << trx.ref_block_num << std::endl;
