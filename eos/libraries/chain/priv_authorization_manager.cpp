...
namespace eosio { namespace chain {
   ...
   priv_authorization_manager::priv_authorization_manager(priv_controller& c, database& d)
   :_control(c),_db(d){}

   void priv_authorization_manager::add_indices() {
      authorization_index_set::add_indices(_db);
   }

   void priv_authorization_manager::initialize_database() {
   ...
   void priv_authorization_manager::add_to_snapshot( const snapshot_writer_ptr& snapshot ) const {
   ...
   void priv_authorization_manager::read_from_snapshot( const snapshot_reader_ptr& snapshot ) {
      authorization_index_set::walk_indices([this, &snapshot]( auto utils ){
         using section_t = typename decltype(utils)::index_t::value_type;
   ...
   const permission_object& priv_authorization_manager::create_permission( account_name account,
   ...
   void priv_authorization_manager::modify_permission( const permission_object& permission, const authority& auth ) {
   ...
   void priv_authorization_manager::remove_permission( const permission_object& permission ) {
   ...
   void priv_authorization_manager::update_permission_usage( const permission_object& permission ) {
   ...
   fc::time_point priv_authorization_manager::get_permission_last_used( const permission_object& permission )const {
      return _db.get<permission_usage_object, by_id>( permission.usage_id ).last_used;
   }

   const permission_object*  priv_authorization_manager::find_permission( const permission_level& level )const
   { try {
   ...
   const permission_object&  priv_authorization_manager::get_permission( const permission_level& level )const
   { try {
      EOS_ASSERT( !level.actor.empty() && !level.permission.empty(), invalid_permission, "Invalid permission" );
      return _db.get<permission_object, by_owner>( boost::make_tuple(level.actor,level.permission) );
   } EOS_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }

   optional<permission_name> priv_authorization_manager::lookup_linked_permission( account_name authorizer_account,
                                                                              account_name scope,
   ...
   optional<permission_name> priv_authorization_manager::lookup_minimum_permission( account_name authorizer_account,
                                                                               account_name scope,
                                                                               action_name act_name
                                                                             )const
   {
   ...
   void priv_authorization_manager::check_updateauth_authorization( const updateauth& update,
   ...
   void priv_authorization_manager::check_deleteauth_authorization( const deleteauth& del,
                                                               const vector<permission_level>& auths
                                                             )const
   {
      EOS_ASSERT( auths.size() == 1, irrelevant_auth_exception,
   ...
   void priv_authorization_manager::check_linkauth_authorization( const linkauth& link,
                                                             const vector<permission_level>& auths
                                                           )const
   {
      EOS_ASSERT( auths.size() == 1, irrelevant_auth_exception,
   ...
   void priv_authorization_manager::check_unlinkauth_authorization( const unlinkauth& unlink,
                                                               const vector<permission_level>& auths
                                                             )const
   {
      EOS_ASSERT( auths.size() == 1, irrelevant_auth_exception,
   ...
   fc::microseconds priv_authorization_manager::check_canceldelay_authorization( const canceldelay& cancel,
                                                                            const vector<permission_level>& auths
                                                                          )const
   {
      EOS_ASSERT( auths.size() == 1, irrelevant_auth_exception,
   ...
   void priv_noop_checktime() {}

   std::function<void()> priv_authorization_manager::_priv_noop_checktime{&priv_noop_checktime};

   void
   priv_authorization_manager::check_authorization(const vector<action> &actions,
   ...
   {
      std::cout << "           E O S I O           :    [Start 1] [priv_authorization_manager] [check_authorization] | " << fc::json::to_string(provided_keys) << std::endl;

      const auto& checktime = ( static_cast<bool>(_checktime) ? _checktime : _priv_noop_checktime );
      ...
      std::cout << "           E O S I O           :    [Start 2] [priv_authorization_manager] [check_authorization] | " << fc::json::to_string(provided_keys) << std::endl;
      ...
      for( const auto& act : actions ) {
         ...
         std::cout << "           E O S I O           :    [Start 3] [priv_authorization_manager] [check_authorization] | " << fc::json::to_string(provided_keys) << std::endl;
   ...
   void
   priv_authorization_manager::check_authorization( account_name                         account,
   ...
   flat_set<public_key_type> priv_authorization_manager::get_required_keys( const transaction& trx,
