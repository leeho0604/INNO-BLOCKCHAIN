...
namespace eosio { namespace chain {
   ...
   const uint32_t priv_block_log::min_supported_version = 1;
    ...
    */
   const uint32_t priv_block_log::max_supported_version = 2;

   namespace detail {
      class priv_block_log_impl {
         public:
            priv_signed_block_ptr         head;
            ...
            bool                     genesis_written_to_priv_block_log = false;
            uint32_t                 version = 0;
      ...
      void priv_block_log_impl::reopen() {
         close();
   ...
   priv_block_log::priv_block_log(const fc::path& data_dir)
   :my(new detail::priv_block_log_impl()) {
   ...
   priv_block_log::priv_block_log(priv_block_log&& other) {
      my = std::move(other.my);
   }

   priv_block_log::~priv_block_log() {
      if (my) {
         flush();
         my->close();
         my.reset();
   ...
   void priv_block_log::open(const fc::path& data_dir) {
      my->close();
      ...
      if (log_size) {
         ...
         EOS_ASSERT( my->version >= min_supported_version && my->version <= max_supported_version, block_log_unsupported_version,
                 ...
                 ("version", my->version)("min", priv_block_log::min_supported_version)("max", priv_block_log::max_supported_version) );


         my->genesis_written_to_priv_block_log = true; // Assume it was constructed properly.
         if (my->version > 1){
   ...
   uint64_t priv_block_log::append(const priv_signed_block_ptr& b) {
      std::cout<< "[ priv_block_log.cpp ][ append ][ START ]" << std::endl;
      try {
         EOS_ASSERT( my->genesis_written_to_priv_block_log, block_log_append_fail, "Cannot append to block log until the genesis is first written" );
         ...
         //          data 암호화해야함       //
         //---------------------------------//
         unsigned int unSize = b->strEncBlock.size();

         //std::cout<< "[ priv_block_log.cpp ][ append ][ size ]:"<< unSize << std::endl;
         //--------------------------------------------//
         //write: priv_signed_block_header
         //b->strEncBlock의 HASH(keccak256) 추출
         Keccak keccak;
         std::string strHash = keccak(b->strEncBlock);
         std::cout<< "[ priv_block_log.cpp ][ HASH ]:" << strHash << std::endl;

         priv_signed_block_header bh(*b);
         
         my->block_stream.write((char*)&bh, sizeof(bh));
         //--------------------------------------------//

         if(unSize < 1){
            unSize = 1;
            std::cout<< "[ priv_block_log.cpp ][ append ]>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" << std::endl;
            std::cout<< "[ priv_block_log.cpp ][ append ][ Error unSize]:"<< unSize << std::endl;
            std::cout<< "[ priv_block_log.cpp ][ append ]>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" << std::endl;
            /*
            //priv_controller.cpp commit_block으로 이동
            ...
            //셋팅 변경하기
            b->strEncBlock = strEnc;
            b->transactions.clear();
            */
         }
         else{
            uint64_t pos2 = my->block_stream.tellp();
            std::cout<< "[ priv_block_log.cpp ][ test ] Num:" << b->block_num() << " | pos1="<< pos2 + 4 <<" | pos2="<< unSize << std::endl;
         }
         my->block_stream.write((char*)&unSize, sizeof(unSize));
         my->block_stream.write(b->strEncBlock.c_str(), unSize);
         ...
         // wook: create action
         // 1: block_id:b->id() & 2: HASH=strHash

         flush();
         std::cout<< "[ priv_block_log.cpp ][ append ][ END ]" << std::endl;
         return pos;
      }
      FC_LOG_AND_RETHROW()
   }

   void priv_block_log::flush() {
      my->block_stream.flush();
      my->index_stream.flush();
   }

   void priv_block_log::reset( const genesis_state& gs, const priv_signed_block_ptr& first_block, uint32_t first_block_num ) {
      std::cout<< "[ priv_block_log.cpp ][ reset ][ START ]" << std::endl;
      my->close();
      ...
      my->genesis_written_to_priv_block_log = true;
      ...
      static_assert( priv_block_log::max_supported_version > 0, "a version number of zero is not supported" );
      my->version = priv_block_log::max_supported_version;
      my->block_stream.seekp( 0 );
      my->block_stream.write( (char*)&my->version, sizeof(my->version) );
      my->block_stream.seekp( pos );

      std::cout<< "[ priv_block_log.cpp ][ reset ][ END ]" << std::endl;
      flush();
   }

   std::pair<priv_signed_block_ptr, uint64_t> priv_block_log::read_block(uint64_t pos)const {
      std::cout<< "[ priv_block_log.cpp ][ read_block ][ START ]" << sizeof(priv_signed_block_header) << std::endl;
      my->check_open_files();

      my->block_stream.seekg(pos);
      std::pair<priv_signed_block_ptr,uint64_t> result;
      //result.first = std::make_shared<priv_signed_block>();
      ...
      */
      std::cout << "[ priv_block_log.cpp ][ read_block ][ CHECK_1 ]" << std::endl;
      //--------------------------------------------//
      //read: priv_signed_block_header
      priv_signed_block_header bh;
      my->block_stream.read ( (char*)&bh, sizeof(priv_signed_block_header) );
      //--------------------------------------------//
      std::cout << "[ priv_block_log.cpp ][ read_block ][ CHECK_2 ]" << std::endl;
      unsigned int unSize = 0;
      my->block_stream.read((char*)&unSize, sizeof(unSize));
      std::cout << "[ priv_block_log.cpp ][ read_block ][ CHECK_3 ]" << std::endl;
      std::string strData = "";
      strData.resize(unSize);

      my->block_stream.read ( &strData.at(0), unSize );
      std::cout << "[ priv_block_log.cpp ][ read_block ][ CHECK_4 ]" << std::endl;
      //my->block_stream.seekg(unSize, std::ios::cur);
      /*
      my->block_stream.read ( &strData.at(0), unSize );
      std::cout << "[ priv_block_log.cpp ][ read_block ][ CHECK_2 ]: "<< strData.size() << std::endl;
      std::string strDec = lc::getRawDecAes256(strData, "12345678901234567890123456789012");
      std::cout << "[ priv_block_log.cpp ][ read_block ][ CHECK_3 ]: " << strDec.size() << std::endl;
      
      //fc::raw::unpack(strDec.c_str(), strDec.size(), *result.first);
      */
      result.first = std::make_shared<priv_signed_block>(bh);
      result.first->strEncBlock = strData;
      result.second = uint64_t(my->block_stream.tellg()) + 8;

      std::cout<< "[ priv_block_log.cpp ][ read_block ][ END ]" << std::endl;
      return result;
   }

   priv_signed_block_ptr priv_block_log::read_block_by_num(uint32_t block_num)const {
      try {
   ...
   uint64_t priv_block_log::get_block_pos(uint32_t block_num) const {
      my->check_open_files();
   ...
   priv_signed_block_ptr priv_block_log::read_head()const {
      my->check_open_files();
   ...
   const priv_signed_block_ptr& priv_block_log::head()const {
      return my->head;
   }

   uint32_t priv_block_log::first_block_num() const {
      return my->first_block_num;
   }

   void priv_block_log::construct_index() {
   ...
   fc::path priv_block_log::repair_log( const fc::path& data_dir, uint32_t truncate_at_block ) {
      ...
      auto priv_block_log_path = blocks_dir / "blocks.log";

      ilog( "Reconstructing '${new_priv_block_log}' from backed up block log", ("new_priv_block_log", priv_block_log_path) );
      ...
      new_block_stream.open( priv_block_log_path.generic_string().c_str(), LOG_WRITE );
      ...
      EOS_ASSERT( version >= min_supported_version && version <= max_supported_version, block_log_unsupported_version,
                 "Unsupported version of block log. Block log version is ${version} while code supports version(s) [${min},${max}]",
                 ("version", version)("min", priv_block_log::min_supported_version)("max", priv_block_log::max_supported_version) );
      ...
      optional<priv_signed_block> bad_block;
   ...
   genesis_state priv_block_log::extract_genesis_state( const fc::path& data_dir ) {
      EOS_ASSERT( fc::is_directory(data_dir) && fc::is_regular_file(data_dir / "blocks.log"), block_log_not_found,
                 "Block log not found in '${blocks_dir}'", ("blocks_dir", data_dir)          );
