...
namespace bacc = boost::accumulators;
   ...
   void transaction_context::init(uint64_t initial_net_usage)
   {
      ...
      //std::cout << "           E O S I O           :    [Start] [transaction_contrxt] [init] [objective_duration_limit] " << fc::variant(objective_duration_limit).as_string() << std::endl;

      _deadline = start + objective_duration_limit;
      //std::cout << "           E O S I O           :    [Start] [transaction_contrxt] [init] [_deadline] " << fc::variant(_deadline).as_string() << std::endl;
   ...
   void transaction_context::schedule_transaction() {
      ...
      const auto& trx_db = control.db().get_index<generated_transaction_multi_index>(); //Not Yet
   ...
   void transaction_context::record_transaction( const transaction_id_type& id, fc::time_point_sec expire ) {
      try {
          ...
          auto* trxId = db.find<transaction_object, by_trx_id>(id);
