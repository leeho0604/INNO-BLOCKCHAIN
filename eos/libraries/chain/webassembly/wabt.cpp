...
class wabt_instantiated_module : public wasm_instantiated_module_interface {
   public:
      ...
      {
         std::cout << "Check Pub ENV " << std::endl;
      ...
      void apply(apply_context& context) override {
         ...
         std::cout << "           E O S I O           :    [Start] [wabt] [apply] || 333 " << context.act.name.to_string() << std::endl;
   ...
   for(auto it = intrinsic_registrator::get_map().begin() ; it != intrinsic_registrator::get_map().end(); ++it) {
      ...
      for(auto itf = it->second.begin(); itf != it->second.end(); ++itf) {
         host_module->AppendFuncExport(itf->first, itf->second.sig, [fn = itf->second.func](const auto *f, const auto *fs, const auto &args, auto &res) {
            //std::cout << "           E O S I O           :    [Start] [AppendFuncExport Call Back ] " << std::endl;

            TypedValue ret = fn(*static_wabt_vars, args);
            //std::cout << "           E O S I O           :    [Start] [pub] [func -> field name ] " << f->field_name << std::endl;
...
// {
//    std::cout << "           E O S I O           :    [Start] [wavt.cpp] [instantiate_module] [code.size]:" << code_size << std::endl;
//    std::unique_ptr<interp::Environment> _env = std::make_unique<interp::Environment>();
//    for (auto it = priv_intrinsic_registrator::get_map().begin(); it != priv_intrinsic_registrator::get_map().end(); ++it)
//    {
//       interp::HostModule *_host_module = _env->AppendHostModule(it->first);
//       //std::cout << "map SIZE:" <<it->second.size() << std::endl;
//       for (auto itf = it->second.begin(); itf != it->second.end(); ++itf)
//       {
...
//    wabt::Result res = ReadBinaryInterp(_env.get(), code_bytes, code_size, read_binary_options, &errors, &instantiated_module);
//    EOS_ASSERT(Succeeded(res), wasm_execution_error, "Error building wabt interp: ${e}", ("e", wabt::FormatErrorsToString(errors, Location::Type::Binary)));

//    return std::make_unique<priv_wabt_instantiated_module>(std::move(_env), initial_memory, instantiated_module);
// }
