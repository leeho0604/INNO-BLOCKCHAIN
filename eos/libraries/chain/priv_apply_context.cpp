...
void priv_apply_context::exec_one( action_trace& trace )
{
   std::cout << "           E O S I O           :    [Start] [priv_apply_context] [exec_one]   " << std::endl;
   ...
   try {
      try {
         ...
         if( native ) {
            ...
            // native is " using native = std::fucntion<apply_context> "
            (*native)( *this );
         }
         ...
         if( a.code.size() > 0
            ...
            try {
               std::cout << "           E O S I O           :    [Start] [priv_apply_context] [exec_one] [a.code_version] " << fc::json::to_string(a.code_version) << std::endl;
               std::cout << "           E O S I O           :    [Start] [priv_apply_context] [exec_one] [a.code.size()] " << a.code.size() << std::endl;
               std::cout << "           E O S I O           :    [Start] [priv_apply_context] [exec_one] [a.abi.size()] " << a.abi.size() << std::endl;
               //control.get_wasm_interface().priv_apply( a.code_version, a.code, *this );
               control.get_wasm_interface().priv_apply(a.code_version, a.code, *this);
            } catch( const priv_wasm_exit& ) {}
         }
   ...
   //std::cout << "           E O S I O           :    [END] [priv_aplly_context] [exec_one]   " << std::endl;
}

void priv_apply_context::finalize_trace( action_trace& trace, const fc::time_point& start )
{
...
void priv_apply_context::exec( action_trace& trace )
{
   std::cout << "           E O S I O           :    [Start] [priv_apply_context] [exec]   " << std::endl;
...
bool priv_apply_context::is_account( const account_name& account )const {
   return nullptr != db.find<account_object,by_name>( account );
}

void priv_apply_context::require_authorization( const account_name& account ) {
...
bool priv_apply_context::has_authorization( const account_name& account )const {
   for( const auto& auth : act.authorization )
...
bool priv_apply_context::has_recipient( account_name code )const {
...
void priv_apply_context::require_recipient( account_name recipient ) {
 ...
 */
void priv_apply_context::execute_inline( action&& a ) {
      ...
      try {
         control.get_authorization_manager()
                .check_authorization( {a},
                                      ...
                                      std::bind(&priv_transaction_context::checktime, &this->trx_context),
                                      false,
...
void priv_apply_context::execute_context_free_inline( action&& a ) {
   auto* code = control.db().find<account_object, by_name>(a.account);
   EOS_ASSERT( code != nullptr, action_validate_exception,
               "inline action's code account ${account} does not exist", ("account", a.account) );
...
void priv_apply_context::schedule_deferred_transaction( const uint128_t& sender_id, account_name payer, transaction&& trx, bool replace_existing ) {
...
bool priv_apply_context::cancel_deferred_transaction( const uint128_t& sender_id, account_name sender ) {
...
const table_id_object* priv_apply_context::find_table( name code, name scope, name table ) {
   return db.find<table_id_object, by_code_scope_table>(boost::make_tuple(code, scope, table));
}

const table_id_object& priv_apply_context::find_or_create_table( name code, name scope, name table, const account_name &payer ) {
...
void priv_apply_context::remove_table( const table_id_object& tid ) {
   update_db_usage(tid.payer, - config::billable_size_v<table_id_object>);
   db.remove(tid);
}

vector<account_name> priv_apply_context::get_active_producers() const {
...
void priv_apply_context::reset_console() {
   _pending_console_output = std::ostringstream();
   _pending_console_output.setf( std::ios::scientific, std::ios::floatfield );
}

bytes priv_apply_context::get_packed_transaction() {
   auto r = fc::raw::pack( static_cast<const transaction&>(trx_context.trx) );
   return r;
}

void priv_apply_context::update_db_usage( const account_name& payer, int64_t delta ) {
   if( delta > 0 ) {
...
int priv_apply_context::get_action( uint32_t type, uint32_t index, char* buffer, size_t buffer_size )const
{
...
int priv_apply_context::get_context_free_data( uint32_t index, char* buffer, size_t buffer_size )const
{
   const auto& trx = trx_context.trx;
...
int priv_apply_context::db_store_i64( uint64_t scope, uint64_t table, const account_name& payer, uint64_t id, const char* buffer, size_t buffer_size ) {
   return db_store_i64( receiver, scope, table, payer, id, buffer, buffer_size);
}

int priv_apply_context::db_store_i64( uint64_t code, uint64_t scope, uint64_t table, const account_name& payer, uint64_t id, const char* buffer, size_t buffer_size ) {
   ...
   const auto &obj = db.create<key_value_object>([&](auto &o) {
      std::cout << "           E O S I O           :    [Start] [priv_apply_context] [db_store_i64] : " << buffer << std::endl;
      o.t_id        = tableid;
      o.primary_key = id;
   ...
   std::cout << "           E O S I O           :    [END] [priv_apply_context] [db_store_i64] : " << buffer << std::endl;
   return keyval_cache.add( obj );
}

void priv_apply_context::db_update_i64( int iterator, account_name payer, const char* buffer, size_t buffer_size ) {
   std::cout << "           E O S I O           :    [Start] [priv_apply_context] [db_update_i64] [payer] " << payer << std::endl;
   std::cout << "           E O S I O           :    [Start] [priv_apply_context] [db_update_i64] [buffer] " << buffer << std::endl;
...
//   require_write_lock( table_obj.scope );
   ...
   db.modify(obj, [&](auto &o) {
      //std::cout << "priv_db_update_i64 : buffer" << buffer<< std::endl;

      o.value.assign(buffer, buffer_size);
      o.payer = payer;
   });
   //std::cout << "           E O S I O           :    [END] [priv_apply_context] [db_update_i64] " << std::endl;
}

void priv_apply_context::db_remove_i64( int iterator ) {
   std::cout << "           E O S I O           :    [START] [apply_context] [db_remove_i64] " << std::endl;
   const key_value_object& obj = keyval_cache.get( iterator );

   const auto& table_obj = keyval_cache.get_table( obj.t_id );
   //std::cout << "priv_db_remove_i64" << obj.id << " : " << obj.payer << " : " << fc::json::to_pretty_string(obj.value) << std::endl;
   EOS_ASSERT( table_obj.code == receiver, table_access_violation, "db access violation" );
...
//   require_write_lock( table_obj.scope );
   ...
   std::cout << "           E O S I O           :    [END] [apply_context] [db_remove_i64] " << std::endl;
}

int priv_apply_context::db_get_i64( int iterator, char* buffer, size_t buffer_size ) {
   const key_value_object& obj = keyval_cache.get( iterator );
...
int priv_apply_context::db_next_i64( int iterator, uint64_t& primary ) {
...
int priv_apply_context::db_previous_i64( int iterator, uint64_t& primary ) {
   const auto& idx = db.get_index<key_value_index, by_scope_primary>();
...
int priv_apply_context::db_find_i64( uint64_t code, uint64_t scope, uint64_t table, uint64_t id ) {
...
int priv_apply_context::db_lowerbound_i64( uint64_t code, uint64_t scope, uint64_t table, uint64_t id ) {
   //require_read_lock( code, scope ); // redundant?
...
int priv_apply_context::db_upperbound_i64( uint64_t code, uint64_t scope, uint64_t table, uint64_t id ) {
   //require_read_lock( code, scope ); // redundant?
...
int priv_apply_context::db_end_i64( uint64_t code, uint64_t scope, uint64_t table ) {
   //require_read_lock( code, scope ); // redundant?
...
uint64_t priv_apply_context::next_global_sequence() {
...
uint64_t priv_apply_context::next_recv_sequence( account_name receiver ) {
...
uint64_t priv_apply_context::next_auth_sequence( account_name actor ) {
...
void priv_apply_context::add_ram_usage( account_name account, int64_t ram_delta ) {
