...
namespace bacc = boost::accumulators;
   ...
   struct priv_deadline_timer_verify {
      priv_deadline_timer_verify() {
   ...
   volatile sig_atomic_t priv_deadline_timer_verify::hit;
   static priv_deadline_timer_verify deadline_timer_verification;

   priv_deadline_timer::priv_deadline_timer() {
      if(initialized)
         return;
      initialized = true;
   ...
   void priv_deadline_timer::start(fc::time_point tp) {
   ...
   void priv_deadline_timer::timer_expired(int) {
      expired = 1;
   }
   volatile sig_atomic_t priv_deadline_timer::expired = 0;
   bool priv_deadline_timer::initialized = false;

   priv_transaction_context::priv_transaction_context( priv_controller& c,
   ...
   void priv_transaction_context::init(uint64_t initial_net_usage)
   {
   ...
   void priv_transaction_context::init_for_implicit_trx( uint64_t initial_net_usage  )
   {
      published = control.pending_block_time();
      init( initial_net_usage);
   }

   void priv_transaction_context::init_for_input_trx( uint64_t packed_trx_unprunable_size,
                                                 uint64_t packed_trx_prunable_size,
                                                 bool skip_recording )
   {
      std::cout << "           E O S I O           :    [Start] [priv_transaction_context] [function] [parameter]  " << std::endl;
      const auto &cfg = control.get_global_properties().configuration;
      ...
      std::cout << "           E O S I O           :    [Start] [priv_transaction_context] [init_for_input_trx] [trx.block_id]  " << std::endl;
   ...
   void priv_transaction_context::init_for_deferred_trx( fc::time_point p )
   {
      published = p;
   ...
   void priv_transaction_context::exec() {
      std::cout << "           E O S I O           :    [Start 1] [priv_transaction_context.cpp] [exec()]  " << std::endl;
      EOS_ASSERT( is_initialized, transaction_exception, "must first initialize" );
      std::cout << "           E O S I O           :    [Start 2] [priv_transaction_context.cpp] [exec()]  " << std::endl;

      if( apply_context_free ) {
         std::cout << "           E O S I O           :    [Start 3] [priv_transaction_context.cpp] [exec()] [apply_context_free]: " << apply_context_free << std::endl;
      ...
      if( delay == fc::microseconds() ) {
         std::cout << "           E O S I O           :    [Start 4] [priv_transaction_context.cpp] [exec()] [delay & fc::microseconds()]: " << variant(delay).as_string() << " : " << variant(fc::microseconds()).as_string() << std::endl;
         for( const auto& act : trx.actions ) {
            trace->action_traces.emplace_back();
            dispatch_action( trace->action_traces.back(), act );
         }
      } else {
         std::cout << "           E O S I O           :    [Start 5] [priv_transaction_context.cpp] [exec()] [schedule_transaction]: " << std::endl;
         schedule_transaction();
      }
   }

   void priv_transaction_context::finalize() {
      EOS_ASSERT( is_initialized, transaction_exception, "must first initialize" );
   ...
   void priv_transaction_context::squash() {
      if (undo_session) undo_session->squash();
   }

   void priv_transaction_context::undo() {
      if (undo_session) undo_session->undo();
   }

   void priv_transaction_context::check_net_usage()const {
      if (!control.skip_trx_checks()) {
   ...
   void priv_transaction_context::checktime()const {
   ...
   void priv_transaction_context::pause_billing_timer() {
   ...
   void priv_transaction_context::resume_billing_timer() {
   ...
   void priv_transaction_context::validate_cpu_usage_to_bill( int64_t billed_us, bool check_minimum )const {
      if (!control.skip_trx_checks()) {
         if( check_minimum ) {
            const auto& cfg = control.get_global_properties().configuration;
   ...
   void priv_transaction_context::add_ram_usage( account_name account, int64_t ram_delta ) {
      auto& rl = control.get_mutable_resource_limits_manager();
   ...
   uint32_t priv_transaction_context::update_billed_cpu_time( fc::time_point now ) {
   ...
   std::tuple<int64_t, int64_t, bool, bool> priv_transaction_context::max_bandwidth_billed_accounts_can_pay( bool force_elastic_limits ) const{
   ...
   void priv_transaction_context::dispatch_action( action_trace& trace, const action& a, account_name receiver, bool context_free, uint32_t recurse_depth ) {
      priv_apply_context  acontext( control, *this, a, recurse_depth );
   ...
   void priv_transaction_context::schedule_transaction() {
   ...
   void priv_transaction_context::record_transaction( const transaction_id_type& id, fc::time_point_sec expire ) {
      try {
   ...
   void priv_transaction_context::validate_referenced_accounts( const transaction& trx, bool enforce_actor_whitelist_blacklist )const {
