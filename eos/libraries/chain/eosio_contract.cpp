...
void validate_authority_precondition( const priv_apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      auto* acct = context.db.find<account_object, by_name>(a.permission.actor);
      EOS_ASSERT( acct != nullptr, action_validate_exception,
                  "account '${account}' does not exist",
...
void apply_eosio_newaccount(apply_context& context) {
   std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [apply_eosio_newaccount] " << std::endl;
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   const auto &new_account = db.create<account_object>([&](auto &a) {
      std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [apply_eosio_newaccount] [ name ] " << fc::json::to_string(create.name) << std::endl;
      a.name = create.name;
...
void priv_apply_eosio_newaccount(priv_apply_context& context) {
   std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [apply_eosio_newaccount] " << std::endl;
   auto create = context.act.data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
...
void apply_eosio_setcode(apply_context& context) {
   std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [apply_eosio_setcode] " << std::endl;

   const auto& cfg = context.control.get_global_properties().configuration;

   auto& db = context.db;
   auto  act = context.act.data_as<setcode>();
   //std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [apply_eosio_setcode][context.act.data_as] " << fc::variant(act).as_string() << std::endl;
   context.require_authorization(act.account);
   std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [apply_eosio_setcode][act.account] " << fc::variant(act.account).as_string() << std::endl;
   ...
   std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [apply_eosio_setcode][act.code.size()] " << (uint32_t)act.code.size() << std::endl;

   const auto& account = db.get<account_object,by_name>(act.account);

   std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [apply_eosio_setcode][code_id] " << fc::variant(code_id).as_string() << std::endl;
   ...
   std::cout << "           E O S I O           :    [END] [eosio_contract.cpp] [apply_eosio_setcode] " << std::endl;
}

void priv_apply_eosio_setcode(priv_apply_context &context)
{
   std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [priv_apply_eosio_setcode] " << std::endl;
   const auto &cfg = context.control.get_global_properties().configuration;
   ...
   //std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [apply_eosio_setcode][context.act.data_as] " << fc::variant(act).as_string() << std::endl;
   context.require_authorization(act.account);
   std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [priv_apply_eosio_setcode][act.account] " << fc::variant(act.account).as_string() << std::endl;
   ...
   {
      code_id = fc::sha256::hash(act.code.data(), (uint32_t)act.code.size());
      priv_wasm_interface::priv_validate(context.control, act.code);
   }
   const auto &account = db.get<account_object, by_name>(act.account);
   int64_t code_size = (int64_t)act.code.size();
   int64_t old_size = (int64_t)account.code.size() * config::setcode_ram_bytes_multiplier;
   ...
   std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [priv_apply_eosio_setcode][code_id] " << fc::variant(code_id).as_string() << std::endl;
   ...
   db.modify(account, [&](auto &a) {
      ...
      a.last_code_update = context.control.pending_block_time();
      a.code_version = code_id;
      std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [priv_apply_eosio_setcode][a.code_version] " << fc::variant(a.code_version).as_string() << std::endl;

      if (code_size > 0)
      {
         a.code.assign(act.code.data(), code_size);
         std::cout << "           E O S I O           :    [Start] [eosio_contract.cpp] [priv_apply_eosio_setcode][a.code.size()] " << a.code.size() << std::endl;
      }
      else
      {
         a.code.resize(0);
   ...
   std::cout << "           E O S I O           :    [END] [eosio_contract.cpp] [priv_apply_eosio_setcode] " << std::endl;
}

void apply_eosio_setabi(apply_context& context) {
   std::cout << "           E O S I O           :    [START ] [eosio_contract.cpp] [apply_eosio_setabi] " << std::endl;
   auto& db  = context.db;
   auto  act = context.act.data_as<setabi>();

   context.require_authorization(act.account);
   std::cout << "           E O S I O           :    [START 2] [eosio_contract.cpp] [apply_eosio_setabi] " << std::endl;
   ...
   db.modify( account, [&]( auto& a ) {
      std::cout << "           E O S I O           :    [START 3] [eosio_contract.cpp] [apply_eosio_setabi] " << std::endl;
   ...
   std::cout << "           E O S I O           :    [END] [eosio_contract.cpp] [apply_eosio_setabi] " << std::endl;
}

void priv_apply_eosio_setabi(priv_apply_context &context)
{
...
void priv_apply_eosio_updateauth(priv_apply_context &context)
{
...
void priv_apply_eosio_deleteauth(priv_apply_context &context)
{
   //   context.require_write_lock( config::eosio_auth_scope );
...
void priv_apply_eosio_linkauth(priv_apply_context &context)
{
   //   context.require_write_lock( config::eosio_auth_scope );
...
void priv_apply_eosio_unlinkauth(priv_apply_context &context)
{
   //   context.require_write_lock( config::eosio_auth_scope );
...
void priv_apply_eosio_canceldelay(priv_apply_context &context)
{
   auto cancel = context.act.data_as<canceldelay>();
   context.require_authorization(cancel.canceling_auth.actor); // only here to mark the single authority on this action as used
