...
namespace eosio { namespace chain {
   ...
   namespace detail { class priv_block_log_impl; }
   ...
   class priv_block_log {
      public:
         priv_block_log(const fc::path& data_dir);
         priv_block_log(priv_block_log&& other);
         ~priv_block_log();

         uint64_t append(const priv_signed_block_ptr& b);
         void flush();
         void reset( const genesis_state& gs, const priv_signed_block_ptr& genesis_block, uint32_t first_block_num = 1 );

         std::pair<priv_signed_block_ptr, uint64_t> read_block(uint64_t file_pos)const;
         priv_signed_block_ptr read_block_by_num(uint32_t block_num)const;
         priv_signed_block_ptr read_block_by_id(const block_id_type& id)const {
            return read_block_by_num(block_header::num_from_id(id));
         }

         /**
          * Return offset of block in file, or priv_block_log::npos if it does not exist.
          */
         uint64_t get_block_pos(uint32_t block_num) const;
         priv_signed_block_ptr        read_head()const;
         const priv_signed_block_ptr& head()const;
      ...
      private:
         ...
         std::unique_ptr<detail::priv_block_log_impl> my;
   };
