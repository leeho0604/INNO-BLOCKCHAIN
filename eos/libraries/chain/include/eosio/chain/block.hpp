...
namespace eosio { namespace chain {
   ...
   struct priv_signed_block : public priv_signed_block_header {
    private:
      priv_signed_block( const priv_signed_block& ) = default;
    public:
      priv_signed_block() = default;
      explicit priv_signed_block( const priv_signed_block_header& h ):priv_signed_block_header(h){ strEncBlock = "";}
      priv_signed_block( priv_signed_block&& ) = default;
      priv_signed_block& operator=(const priv_signed_block&) = delete;
      priv_signed_block clone() const { return *this; }

      vector<transaction_receipt>   transactions; /// new or generated transactions
      extensions_type               block_extensions;
      std::string                   strEncBlock;
   };
   using priv_signed_block_ptr = std::shared_ptr<priv_signed_block>;
...
FC_REFLECT_DERIVED(eosio::chain::priv_signed_block, (eosio::chain::priv_signed_block_header), (transactions)(block_extensions)(strEncBlock) )
