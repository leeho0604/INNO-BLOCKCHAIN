...
namespace eosio { namespace chain {
   ...
   struct wasm_interface_impl {
      ...
      {
         std::cout << "           E O S I O           :    [START] [wasm_interface_private] [get_instantiated_module]:" << std::endl;

         auto it = instantiation_cache.find(code_id);
         std::cout << "           E O S I O           :    [START 1] [wasm_interface_private] [get_instantiated_module]:" << std::endl;

         if(it == instantiation_cache.end()) {
            std::cout << "           E O S I O           :    [START 2] [wasm_interface_private] [get_instantiated_module]:" << std::endl;

            auto timer_pause = fc::make_scoped_exit([&](){
               trx_context.resume_billing_timer();
            });
            trx_context.pause_billing_timer();
            std::cout << "           E O S I O           :    [START 3] [wasm_interface_private] [get_instantiated_module]:" << std::endl;

            IR::Module module;
            try {
               std::cout << "           E O S I O           :    [START 4] [wasm_interface_private] [get_instantiated_module]:" << std::endl;
            ...
            std::cout << "           E O S I O           :    [START 5] [wasm_interface_private] [get_instantiated_module]:" << std::endl;

            wasm_injections::wasm_binary_injection injector(module);
            injector.inject();
            std::cout << "           E O S I O           :    [START 6] [wasm_interface_private] [get_instantiated_module]:" << std::endl;

            std::vector<U8> bytes;
            try {
               std::cout << "           E O S I O           :    [START 7] [wasm_interface_private] [get_instantiated_module]:" << std::endl;

               Serialization::ArrayOutputStream outstream;
               WASM::serialize(outstream, module);
               bytes = outstream.getBytes();
               std::cout << "           E O S I O           :    [START 8] [wasm_interface_private] [get_instantiated_module]:" << std::endl;
            } catch(const Serialization::FatalSerializationException& e) {
               EOS_ASSERT(false, wasm_serialization_error, e.message.c_str());
            } catch(const IR::ValidationException& e) {
               EOS_ASSERT(false, wasm_serialization_error, e.message.c_str());
            }
                           std::cout << "           E O S I O           :    [START] [wasm_interface_private] [pub_code_size]:" << bytes.size() << std::endl;
         ...
         std::cout << "           E O S I O           :    [END] [wasm_interface_private] [get_instantiated_module]:" << std::endl;
         return it->second;
      }
...
#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG) \
   ...
   //_REGISTER_PRIV_WABT_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
