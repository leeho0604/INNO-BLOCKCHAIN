...
class wavm_runtime : public eosio::chain::priv_wasm_runtime_interface
{
public:
   wavm_runtime();
   ~wavm_runtime();
   std::unique_ptr<priv_wasm_instantiated_module_interface> instantiate_module(const char *code_bytes, size_t code_size, std::vector<uint8_t> initial_memory) ;
...
struct running_instance_priv_context {
   MemoryInstance* memory;
   priv_apply_context* priv_apply_ctx;
};
extern running_instance_priv_context the_running_instance_priv_context;
 ...
 */
template <typename T>
inline array_ptr<T> array_ptr_impl(running_instance_priv_context &ctx, U32 ptr, size_t length)
{
 ...
 */
inline null_terminated_ptr null_terminated_ptr_impl(running_instance_priv_context& ctx, U32 ptr)
{
   MemoryInstance* mem = ctx.memory;
   if(!mem)
      Runtime::causeException(Exception::Cause::accessViolation);
...
template<typename T>
inline auto convert_native_to_wasm(const running_instance_priv_context& ctx, T val) {
   return native_to_wasm_t<T>(val);
}

inline auto convert_native_to_wasm(const running_instance_priv_context& ctx, const name &val) {
   return native_to_wasm_t<const name &>(val.value);
}

inline auto convert_native_to_wasm(const running_instance_priv_context& ctx, const fc::time_point_sec& val) {
   return native_to_wasm_t<const fc::time_point_sec &>(val.sec_since_epoch());
}

inline auto convert_native_to_wasm(running_instance_priv_context& ctx, char* ptr) {
   MemoryInstance* mem = ctx.memory;
   if(!mem)
      Runtime::causeException(Exception::Cause::accessViolation);
...
struct intrinsic_invoker_impl<Ret, std::tuple<>, std::tuple<Translated...>> {
   using next_method_type        = Ret (*)(running_instance_priv_context &, Translated...);

   template<next_method_type Method>
   static native_to_wasm_t<Ret> invoke(Translated... translated) {
      return convert_native_to_wasm(the_running_instance_priv_context, Method(the_running_instance_priv_context, translated...));
   }
...
struct intrinsic_invoker_impl<void_type, std::tuple<>, std::tuple<Translated...>> {
   using next_method_type        = void_type (*)(running_instance_priv_context &, Translated...);

   template<next_method_type Method>
   static void invoke(Translated... translated) {
      Method(the_running_instance_priv_context, translated...);
   }
...
struct intrinsic_invoker_impl<Ret, std::tuple<Input, Inputs...>, std::tuple<Translated...>> {
   ...
   using then_type = Ret (*)(running_instance_priv_context &, Input, Inputs..., Translated...);

   template<then_type Then>
   static Ret translate_one(running_instance_priv_context& ctx, Inputs... rest, Translated... translated, translated_type last) {
...
struct intrinsic_invoker_impl<Ret, std::tuple<array_ptr<T>, size_t, Inputs...>, std::tuple<Translated...>> {
   ...
   using then_type = Ret(*)(running_instance_priv_context&, array_ptr<T>, size_t, Inputs..., Translated...);

   template<then_type Then, typename U=T>
   static auto translate_one(running_instance_priv_context& ctx, Inputs... rest, Translated... translated, I32 ptr, I32 size) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      ...
      if ( reinterpret_cast<uintptr_t>(base) % alignof(T) != 0 ) {
         if(ctx.priv_apply_ctx->control.contracts_console())
...
struct intrinsic_invoker_impl<Ret, std::tuple<null_terminated_ptr, Inputs...>, std::tuple<Translated...>> {
   ...
   using then_type = Ret(*)(running_instance_priv_context&, null_terminated_ptr, Inputs..., Translated...);

   template<then_type Then>
   static Ret translate_one(running_instance_priv_context& ctx, Inputs... rest, Translated... translated, I32 ptr) {
...
struct intrinsic_invoker_impl<Ret, std::tuple<array_ptr<T>, array_ptr<U>, size_t, Inputs...>, std::tuple<Translated...>> {
   ...
   using then_type = Ret(*)(running_instance_priv_context&, array_ptr<T>, array_ptr<U>, size_t, Inputs..., Translated...);

   template<then_type Then>
   static Ret translate_one(running_instance_priv_context& ctx, Inputs... rest, Translated... translated, I32 ptr_t, I32 ptr_u, I32 size) {
...
struct intrinsic_invoker_impl<Ret, std::tuple<array_ptr<char>, int, size_t>, std::tuple<>> {
   ...
   using then_type = Ret(*)(running_instance_priv_context&, array_ptr<char>, int, size_t);

   template<then_type Then>
   static Ret translate_one(running_instance_priv_context& ctx, I32 ptr, I32 value, I32 size) {
      const auto length = size_t(size);
...
struct intrinsic_invoker_impl<Ret, std::tuple<T *, Inputs...>, std::tuple<Translated...>> {
   using next_step = intrinsic_invoker_impl<Ret, std::tuple<Inputs...>, std::tuple<Translated..., I32>>;
   using then_type = Ret (*)(running_instance_priv_context&, T *, Inputs..., Translated...);

   template<then_type Then, typename U=T>
   static auto translate_one(running_instance_priv_context& ctx, Inputs... rest, Translated... translated, I32 ptr) -> std::enable_if_t<std::is_const<U>::value, Ret> {
...
struct intrinsic_invoker_impl<Ret, std::tuple<const name&, Inputs...>, std::tuple<Translated...>> {
   ...
   using then_type = Ret (*)(running_instance_priv_context&, const name&, Inputs..., Translated...);

   template<then_type Then>
   static Ret translate_one(running_instance_priv_context& ctx, Inputs... rest, Translated... translated, native_to_wasm_t<const name&> wasm_value) {
...
struct intrinsic_function_invoker {
   ...
   static Ret wrapper(running_instance_priv_context& ctx, Params... params) {
      class_from_priv_wasm<Cls>::value(*ctx.priv_apply_ctx).checktime();
      return (class_from_priv_wasm<Cls>::value(*ctx.priv_apply_ctx).*Method)(params...);
   }
...
struct intrinsic_function_invoker<WasmSig, void, MethodSig, Cls, Params...> {
   ...
   template<MethodSig Method>
   static void_type wrapper(running_instance_priv_context& ctx, Params... params) {
      class_from_priv_wasm<Cls>::value(*ctx.priv_apply_ctx).checktime();
      (class_from_priv_wasm<Cls>::value(*ctx.priv_apply_ctx).*Method)(params...);
      return void_type();
   }
...
#define _REGISTER_WAVM_PRIV_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static Intrinsics::FunctionPR _INTRINSIC_NAME(__intrinsic_fn, __COUNTER__) (\
      MOD "." NAME,\
