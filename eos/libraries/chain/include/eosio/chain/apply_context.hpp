   ...
   private:
      ...
      class iterator_cache {
         ...
         private:
            ...
            // ------------ priv -----------
            // map<table_id_object::id_type, pair<const table_id_object*, int>> _my_table_cache;
            // vector<const table_id_object*>                  _my_end_iterator_to_table;
            // vector<const T*>                                _my_iterator_to_object;
            // map<const T*,int>                               _my_object_to_iterator;
...
//             int priv_store( uint64_t scope, uint64_t table, const account_name& payer,
//                        uint64_t id, secondary_key_proxy_const_type value )
//             {
//                std::cout << "wook : apply_context.hpp : priv_store 1 " << std::endl;
//                EOS_ASSERT( payer != account_name(), invalid_table_payer, "must specify a valid account to pay for new record" );

// //               context.require_write_lock( scope );

//                const auto& tab = context.priv_find_or_create_table( context.receiver, scope, table, payer );

//                const auto& obj = context.priv_db.create<ObjectType>( [&]( auto& o ){
//                   o.t_id          = tab.id;
//                   o.primary_key   = id;
//                   secondary_key_helper_t::set(o.secondary_key, value);
//                   o.payer         = payer;
...
//                context.priv_db.modify( tab, [&]( auto& t ) {
//                  ++t.count;
//                });
...
//                context.priv_db.modify( table_obj, [&]( auto& t ) {
//                   --t.count;
//                });
//                context.priv_db.remove( obj );

//                if (table_obj.count == 0) {
//                   context.priv_remove_table(table_obj);
//                }
...
//             void priv_update( int iterator, account_name payer, secondary_key_proxy_const_type secondary ) {
//                const auto& obj = itr_cache.get( iterator );
...
//                context.priv_db.modify( obj, [&]( auto& o ) {
//                  secondary_key_helper_t::set(o.secondary_key, secondary);
//                  o.payer = payer;
//                });
//             }
            ...
            // int priv_find_primary( uint64_t code, uint64_t scope, uint64_t table, secondary_key_proxy_type secondary, uint64_t primary ) {
            //    auto tab = context.priv_find_table( code, scope, table );
            //    if( !tab ) return -1;

            //    auto table_end_itr = itr_cache.cache_table( *tab );

            //    const auto* obj = context.priv_db.find<ObjectType, by_primary>( boost::make_tuple( tab->id, primary ) );
            //    if( !obj ) return table_end_itr;
            //    secondary_key_helper_t::get(secondary, obj->secondary_key);
   ...
   public:
      ...
      int  priv_db_get_i64( int iterator, char* buffer, size_t buffer_size );
      int  db_next_i64( int iterator, uint64_t& primary );
      int  priv_db_next_i64(int iterator, uint64_t &primary);
      ...
      int  priv_db_store_i64(uint64_t scope, uint64_t table, const account_name &payer, uint64_t id, const char *buffer, size_t buffer_size);
      void priv_db_update_i64(int iterator, account_name payer, const char *buffer, size_t buffer_size);
      void priv_db_remove_i64(int iterator);
   private:

      const table_id_object* find_table( name code, name scope, name table );
      const table_id_object* priv_find_table( name code, name scope, name table );
      const table_id_object& find_or_create_table( name code, name scope, name table, const account_name &payer );
      const table_id_object& priv_find_or_create_table(name code, name scope, name table, const account_name &payer);
      void                   remove_table( const table_id_object& tid );
      void                   priv_remove_table( const table_id_object& tid );
      ...
      int  priv_db_store_i64(uint64_t code, uint64_t scope, uint64_t table, const account_name &payer, uint64_t id, const char *buffer, size_t buffer_size);
   ...
   private:
      ...
      //iterator_cache<key_value_object>    priv_keyval_cache;   // using db_store and update function 
