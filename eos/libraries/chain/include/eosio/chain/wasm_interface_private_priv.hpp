...
namespace eosio { namespace chain {
   ...
   struct priv_wasm_interface_impl {
      priv_wasm_interface_impl(priv_wasm_interface::vm_type vm) {
         //if(vm == priv_wasm_interface::vm_type::wavm)
            //priv_runtime_interface = std::make_unique<webassembly::wavm::wavm_runtime>();
         if(vm == priv_wasm_interface::vm_type::wabt)
            priv_runtime_interface = std::make_unique<webassembly::wabt_runtime::priv_wabt_runtime>();
         else
      ...
      std::unique_ptr<priv_wasm_instantiated_module_interface>& get_instantiated_module( const digest_type& code_id,
                                                                                    const shared_string& code,
                                                                                    priv_transaction_context& trx_context )
      {
         ...
         if(it == instantiation_cache.end()) {
            ...
            std::cout << "           E O S I O           :    [START] [wasm_interface_private] [priv_code_size]:" << bytes.size() << std::endl;

            it = instantiation_cache.emplace(code_id, priv_runtime_interface->priv_instantiate_module((const char *)bytes.data(), bytes.size(), parse_initial_memory(module))).first;
         }
         return it->second;
      }


      std::unique_ptr<priv_wasm_runtime_interface> priv_runtime_interface;
      map<digest_type, std::unique_ptr<priv_wasm_instantiated_module_interface>> instantiation_cache;
   };

#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG) \
   _REGISTER_WABT_PRIV_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)    \
   _REGISTER_WAVM_PRIV_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
