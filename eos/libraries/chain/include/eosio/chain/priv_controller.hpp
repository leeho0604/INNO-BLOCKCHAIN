...
namespace eosio { namespace chain {
   ...
   class priv_authorization_manager;
   ...
   using priv_apply_handler = std::function<void(priv_apply_context&)>;
   using unapplied_transactions_type = map<transaction_id_type, transaction_metadata_ptr>;

   class priv_fork_database;

   enum class priv_db_read_mode {
      SPECULATIVE,
      HEAD,
      READ_ONLY,
      IRREVERSIBLE
   ...
   enum class priv_validation_mode {
      FULL,
      LIGHT
   };

   struct detach_key_data{
      fc::optional<block_id_type> blockID;
      //block_id_type blockID;
      std::vector<std::string> vecDetKeys;
   };
   using detach_key_data_ptr = std::shared_ptr<detach_key_data>;

   class priv_controller {
      public:
         ...
         struct config {
            ...
            priv_wasm_interface::vm_type priv_wasm_runtime = chain::config::priv_default_wasm_runtime; // PRIV

            priv_db_read_mode             read_mode              = priv_db_read_mode::SPECULATIVE;
            priv_validation_mode          block_validation_mode  = priv_validation_mode::FULL;

            flat_set<account_name>   resource_greylist;
            flat_set<account_name>   trusted_producers;
         };

         enum class priv_block_status {
         ...
         explicit priv_controller( const config& cfg );
         ~priv_controller();
         ...
         const priv_fork_database& fork_db()const;
         ...
         const priv_authorization_manager&          get_authorization_manager()const;
         priv_authorization_manager&                get_mutable_authorization_manager();
         ...
         priv_block_state_ptr      head_block_state()const;
         ...
         priv_block_state_ptr         pending_block_state()const;
         ...
         priv_signed_block_ptr fetch_block_by_number( uint32_t block_num )const;
         priv_signed_block_ptr fetch_block_by_id( block_id_type id )const;

         priv_block_state_ptr fetch_block_state_by_number( uint32_t block_num )const;
         priv_block_state_ptr fetch_block_state_by_id( block_id_type id )const;
         ...
         bool skip_db_sessions( priv_block_status bs )const;
         ...
         priv_db_read_mode get_read_mode()const;
         priv_validation_mode get_validation_mode()const;

         void set_subjective_cpu_leeway(fc::microseconds leeway);
         // keys store
         void insertSharmirKeys(fc::optional<block_id_type> &block_id, std::string &sharedKey);
         // keys remove
         void removeSharmirKeys(fc::optional<block_id_type> &blockId, std::string &sharedKey);
         // keys find
         const account_object* findAccountObject();
         const sss_object* findSharmirKeys(fc::optional<block_id_type> &blockId, std::string &sharedKey);

         signal<void(const priv_signed_block_ptr&)>         pre_accepted_block;
         signal<void(const priv_block_state_ptr&)>          accepted_block_header;
         signal<void(const priv_block_state_ptr&)>          accepted_block;

         // This signal is sending data for all nodes and saved in std::map;
         signal<void(const detach_key_data_ptr&)>          accepted_detach_keys;
         
         signal<void(const priv_block_state_ptr&)>          irreversible_block;
         signal<void(const transaction_metadata_ptr&)> priv_accepted_transaction;
         signal<void(const transaction_trace_ptr&)>    priv_applied_transaction;
         ...
         */
         ...
         const priv_apply_handler* find_apply_handler( account_name contract, scope_name scope, action_name act )const;
         priv_wasm_interface& get_wasm_interface();
         ...
         //original private function
         chainbase::database& mutable_db()const;
      private:
         friend class priv_apply_context;
         friend class transaction_context;

         

         std::unique_ptr<priv_controller_impl> my;

   };

} }  /// eosio::chain

FC_REFLECT( eosio::chain::priv_controller::config,
            (actor_whitelist)
            (actor_blacklist)
            (contract_whitelist)
            (contract_blacklist)
...
FC_REFLECT(eosio::chain::detach_key_data,  (blockID)(vecDetKeys) )