   ...
   public:
      ...
      class generic_index
      {
         public:
            ...
            generic_index( priv_apply_context& c ):context(c){}
...
//               context.require_write_lock( table_obj.scope );
   ...
   /// Constructor
   public:
      priv_apply_context(priv_controller& con, priv_transaction_context& trx_ctx, const action& a, uint32_t depth=0)
      :control(con)
      ,db(con.mutable_db())
      ,trx_context(trx_ctx)
      ,act(a)
   ...
   public:
      ...
      priv_controller&                   control;
      chainbase::database&          db;  ///< database where state is stored
      priv_transaction_context&          trx_context; ///< transaction context in which the action is running
...
//FC_REFLECT(eosio::chain::priv_apply_context::apply_results, (applied_actions)(deferred_transaction_requests)(deferred_transactions_count))
