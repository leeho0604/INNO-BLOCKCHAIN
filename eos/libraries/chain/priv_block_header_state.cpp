...
namespace eosio { namespace chain {
   ...
   bool priv_block_header_state::is_active_producer( account_name n )const {
      return producer_to_last_produced.find(n) != producer_to_last_produced.end();
   }

   producer_key priv_block_header_state::get_scheduled_producer( block_timestamp_type t )const {
      auto index = t.slot % (active_schedule.producers.size() * config::producer_repetitions);
      index /= config::producer_repetitions;
      return active_schedule.producers[index];
   }

   producer_key priv_block_header_state::get_scheduled_producer_name()const{
      // for(auto& a : active_schedule.producers)
      // {
      //    std::cout <<"Wook : get_scheduled_producer_name : producer_name : " << a.producer_name << std::endl;
      // }
      return active_schedule.producers[0];
   }

   bool priv_block_header_state::bp_check(account_name& name)const{
      for(auto& a : active_schedule.producers)
         if(a.producer_name == name){
            std::cout << "           E O S I O           :    [Start] [priv_block_header_state] [bp_check] [same bp]  " << std::endl;
            return true;
         }
         std::cout << "           E O S I O           :    [Start] [priv_block_header_state] [bp_check] [not the same]  " << std::endl;
         return false;
   }

   uint32_t priv_block_header_state::calc_dpos_last_irreversible()const {
   ...
   */
  priv_block_header_state priv_block_header_state::generate_next( block_timestamp_type when )const {
    priv_block_header_state result;
   ...
   bool priv_block_header_state::maybe_promote_pending() {
  ...
  void priv_block_header_state::set_new_producers( producer_schedule_type pending ) {
   ...
   */
  priv_block_header_state priv_block_header_state::next( const priv_signed_block_header& h, bool skip_validate_signee )const {
  ...
  void priv_block_header_state::set_confirmed( uint16_t num_prev_blocks ) {
     /*
  ...
  digest_type   priv_block_header_state::sig_digest()const {
  ...
  void priv_block_header_state::sign( const std::function<signature_type(const digest_type&)>& signer ) {
     auto d = sig_digest();
  ...
  public_key_type priv_block_header_state::signee()const {
    return fc::crypto::public_key( header.producer_signature, sig_digest(), true );
  }

  void priv_block_header_state::verify_signee( const public_key_type& signee )const {
  ...
  void priv_block_header_state::add_confirmation( const header_confirmation& conf ) {
