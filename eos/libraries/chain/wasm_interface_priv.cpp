...
#include <eosio/chain/wasm_interface_private_priv.hpp>
...
namespace eosio { namespace chain {
   ...
   priv_wasm_interface::priv_wasm_interface(vm_type vm) : my(new priv_wasm_interface_impl(vm)) {}

   priv_wasm_interface::~priv_wasm_interface() {}


    void priv_wasm_interface::priv_validate(const priv_controller &control, const bytes &code)
    {
       Module module;
       try
       {
       ...
       std::cout << "priv_validate : code_size : " << code.size() << std::endl;
       wasm_validations::wasm_binary_validation validator(control, module);
       validator.validate();

       priv_root_resolver resolver(true);
       LinkResult link_result = privLinkModule(module, resolver);
       std::cout << "           E O S I O           :    [Start] [wasm_interface_priv.cpp] [priv_validation][ TRUE ] " << std::endl;
   ...
   void priv_wasm_interface::priv_apply( const digest_type& code_id, const shared_string& code, priv_apply_context& priv_context ) {
      std::cout << "           E O S I O           :    [Start] [priv_wasm_interface] [priv_apply]:" << code.size() << std::endl;
      // my->get_instantiated_module(code_id, code, priv_context.trx_context)->priv_apply(priv_context);
      my->get_instantiated_module(code_id, code, priv_context.trx_context)->priv_apply(priv_context);
   }

   void priv_wasm_interface::exit()
   {
      my->priv_runtime_interface->priv_immediately_exit_currently_running_module();
   }

   priv_wasm_instantiated_module_interface::~priv_wasm_instantiated_module_interface() {}
   priv_wasm_runtime_interface::~priv_wasm_runtime_interface() {}

#if defined(assert)
   #undef assert
#endif

class priv_context_aware_api
{
public:

   priv_context_aware_api(priv_apply_context& ctx, bool context_free = false)
       : priv_context(ctx)
   {
      if (priv_context.context_free)
         EOS_ASSERT(context_free, unaccessible_api, "only priv_context free api's can be used in this priv_context");
      priv_context.used_context_free_api |= !context_free;
   }

   void checktime()
   {
      priv_context.trx_context.checktime();
   }

protected:
   priv_apply_context&           priv_context;
};

class priv_context_free_api : public priv_context_aware_api
{
public:
   priv_context_free_api(priv_apply_context &ctx)
       : priv_context_aware_api(ctx, true)
   {
      /* the context_free_data is not available during normal application because it is prunable */
      EOS_ASSERT(priv_context.context_free, unaccessible_api, "this API may only be called from context_free apply");
   }

   int get_context_free_data(uint32_t index, array_ptr<char> buffer, size_t buffer_size) const
   {
      return priv_context.get_context_free_data(index, buffer, buffer_size);
   }
};

class priv_privileged_api : public priv_context_aware_api
{
public:
   priv_privileged_api(priv_apply_context &ctx)
       : priv_context_aware_api(ctx)
   {
      EOS_ASSERT(priv_context.privileged, unaccessible_api, "${code} does not have permission to call this API", ("code", priv_context.receiver));
   }
   ...
   void set_resource_limits(account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight)
   {
      ...
      if (priv_context.control.get_mutable_resource_limits_manager().set_account_limits(account, ram_bytes, net_weight, cpu_weight))
      {
         priv_context.trx_context.validate_ram_usage.insert(account);
      }
   }

   void get_resource_limits(account_name account, int64_t &ram_bytes, int64_t &net_weight, int64_t &cpu_weight)
   {
      priv_context.control.get_resource_limits_manager().get_account_limits(account, ram_bytes, net_weight, cpu_weight);
   }
   ...
   int64_t set_proposed_producers(array_ptr<char> packed_producer_schedule, size_t datalen)
   {
      ...
      {
         EOS_ASSERT(priv_context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account");
      ...
      return priv_context.control.set_proposed_producers(std::move(producers));
   }

   uint32_t get_blockchain_parameters_packed(array_ptr<char> packed_blockchain_parameters, size_t buffer_size)
   {
      auto &gpo = priv_context.control.get_global_properties();
   ...
   void set_blockchain_parameters_packed(array_ptr<char> packed_blockchain_parameters, size_t datalen)
   {
      ...
      cfg.validate();
      priv_context.db.modify(priv_context.control.get_global_properties(),
                        [&](auto &gprops) {
   ...
   {
      return priv_context.db.get<account_object, by_name>(n).privileged;
   }

   void set_privileged(account_name n, bool is_priv)
   {
      const auto &a = priv_context.db.get<account_object, by_name>(n);
      priv_context.db.modify(a, [&](auto &ma) {
         ma.privileged = is_priv;
      });
   }
};

class priv_softfloat_api : public priv_context_aware_api
{
public:
   // TODO add traps on truncations for special cases (NaN or outside the range which rounds to an integer)
   priv_softfloat_api(priv_apply_context &ctx)
       : priv_context_aware_api(ctx, true) {}
...
class priv_producer_api : public priv_context_aware_api
{
public:
   using priv_context_aware_api::priv_context_aware_api;

   int get_active_producers(array_ptr<chain::account_name> producers, size_t buffer_size)
   {
      auto active_producers = priv_context.get_active_producers();
...
class priv_crypto_api : public priv_context_aware_api
{
public:
   explicit priv_crypto_api(priv_apply_context &ctx)
       : priv_context_aware_api(ctx, true) {}
   /**
...
class priv_permission_api : public priv_context_aware_api
{
public:
   using priv_context_aware_api::priv_context_aware_api;
                                        ...
                                        array_ptr<char> perms_data, size_t perms_size)
   {
      ...
      try
      {
         priv_context.control.get_authorization_manager().check_authorization(trx.actions,
                                                                              provided_keys,
                                                                              provided_permissions,
                                                                              fc::seconds(trx.delay_sec),
                                                                              std::bind(&priv_transaction_context::checktime, &priv_context.trx_context),
                                                                              false);
                                       ...
                                       uint64_t delay_us)
   {
      ...
      {
         const auto &am = priv_context.control.get_authorization_manager();
      ...
      {
         auto *acct = priv_context.db.find<account_object, by_name>(account);
...
class priv_authorization_api : public priv_context_aware_api
{
public:
   using priv_context_aware_api::priv_context_aware_api;
   ...
   {
      priv_context.require_authorization(account);
   }

   bool has_authorization(const account_name &account) const
   {
      return priv_context.has_authorization(account);
   }

   void require_authorization(const account_name &account,
                              const permission_name &permission)
   {
      priv_context.require_authorization(account, permission);
   }

   void require_recipient(account_name recipient)
   {
      priv_context.require_recipient(recipient);
   }

   bool is_account(const account_name &account) const
   {
      return priv_context.is_account(account);
   }
};

class priv_system_api : public priv_context_aware_api
{
public:
   using priv_context_aware_api::priv_context_aware_api;
   ...
   {
      return static_cast<uint64_t>(priv_context.control.pending_block_time().time_since_epoch().count());
   }

   uint64_t publication_time()
   {
      return static_cast<uint64_t>(priv_context.trx_context.published.time_since_epoch().count());
   }
};

constexpr size_t max_assert_message = 1024;

class priv_context_free_system_api : public priv_context_aware_api
{
public:
   explicit priv_context_free_system_api(priv_apply_context &ctx)
       : priv_context_aware_api(ctx, true) {}
   ...
   {
      priv_context.control.get_wasm_interface().exit();
   }
};

class priv_action_api : public priv_context_aware_api
{
public:
   priv_action_api(priv_apply_context &ctx)
       : priv_context_aware_api(ctx, true) {}

   int read_action_data(array_ptr<char> memory, size_t buffer_size)
   {
      std::cout << "           E O S I O           :    [Start] [wasm_interface_priv] [read_action_data] " << priv_context.act.data.size() << std::endl;

      auto s = priv_context.act.data.size();
      if (buffer_size == 0)
         return s;

      auto copy_size = std::min(buffer_size, s);
      memcpy(memory, priv_context.act.data.data(), copy_size);
      std::cout << "           E O S I O           :    [END] [wasm_interface_priv] [read_action_data] " << priv_context.act.data.size() << std::endl;
      return copy_size;
   }

   int action_data_size()
   {
      //std::cout << "priv action_data_size" << priv_context.act.data.size() << std::endl;
      std::cout << "           E O S I O           :    [Start] [wasm_interface_priv] [action_data_size] [priv_context.act.data.size]  " << priv_context.act.data.size() << std::endl;
      std::cout << "           E O S I O           :    [END] [wasm_interface_priv] [action_data_size] [priv_context.act.data.size]  " << priv_context.act.data.size() << std::endl;

      return priv_context.act.data.size();
   }
...
//    int priv_read_action_data(array_ptr<char> memory, size_t buffer_size)
//    {
//       std::cout << "[Start] read_action_data" << std::endl;
//       auto s = priv_context.act.data.size();
//       if (buffer_size == 0)
//          return s;
...
//       auto copy_size = std::min(buffer_size, s);
//       memcpy(memory, priv_context.act.data.data(), copy_size);
//       std::cout << "[END] read_action_data" << std::endl;
//       return copy_size;
//    }

//    int priv_action_data_size()
//    {
//       std::cout << "action_data_size" << priv_context.act.data.size() << std::endl;
//       return priv_context.act.data.size();
//    }

//    name priv_current_receiver()
//    {
//       return priv_context.receiver;
//    }
// };

class priv_console_api : public priv_context_aware_api
{
public:
   priv_console_api(priv_apply_context &ctx)
       : priv_context_aware_api(ctx, true), ignore(!ctx.control.contracts_console()) {}
   ...
   void prints(null_terminated_ptr str)
   {
      if (!ignore)
      {
         priv_context.console_append<const char *>(str);
      }
   }
   ...
   void prints_l(array_ptr<const char> str, size_t str_len)
   {
      if (!ignore)
      {
         priv_context.console_append(string(str, str_len));
      }
   }
   ...
   void printi(int64_t val)
   {
      if (!ignore)
      {
         priv_context.console_append(val);
      }
   }
   ...
   void printi128(const __int128 &val)
   {
      if (!ignore)
      {
         ...
         if (is_negative)
         {
            priv_context.console_append("-");
         }

         priv_context.console_append(fc::variant(v).get_string());
      }
   }
   ...
   void printsf(float val)
   {
      if (!ignore)
      {
         ...
         auto &console = priv_context.get_console_stream();
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
         priv_context.console_append(*(long double *)(&val_approx));
#pragma GCC diagnostic pop
#else
         console.precision(std::numeric_limits<double>::digits10);
         double val_approx = from_softfloat64(f128M_to_f64(&val));
         priv_context.console_append(val_approx);
#endif
         console.precision(orig_prec);
      }
   }
   ...
   void printn(const name &value)
   {
      if (!ignore)
      {
         priv_context.console_append(value.to_string());
      }
   }
   ...
   void printhex(array_ptr<const char> data, size_t data_len)
   {
      if (!ignore)
      {
         priv_context.console_append(fc::to_hex(data, data_len));
      }
   }
...
#define DB_API_METHOD_WRAPPERS_SIMPLE_SECONDARY(IDX, TYPE)                                                                \
   ...
   {                                                                                                                      \
      return priv_context.IDX.store(scope, table, payer, id, secondary);                                                       \
   }                                                                                                                      \
   void db_##IDX##_update(int iterator, uint64_t payer, const TYPE &secondary)                                            \
   {                                                                                                                      \
      return priv_context.IDX.update(iterator, payer, secondary);                                                              \
   }                                                                                                                      \
   void db_##IDX##_remove(int iterator)                                                                                   \
   {                                                                                                                      \
      return priv_context.IDX.remove(iterator);                                                                                \
   }                                                                                                                      \
   int db_##IDX##_find_secondary(uint64_t code, uint64_t scope, uint64_t table, const TYPE &secondary, uint64_t &primary) \
   {                                                                                                                      \
      return priv_context.IDX.find_secondary(code, scope, table, secondary, primary);                                          \
   }                                                                                                                      \
   int db_##IDX##_find_primary(uint64_t code, uint64_t scope, uint64_t table, TYPE &secondary, uint64_t primary)          \
   {                                                                                                                      \
      return priv_context.IDX.find_primary(code, scope, table, secondary, primary);                                            \
   }                                                                                                                      \
   int db_##IDX##_lowerbound(uint64_t code, uint64_t scope, uint64_t table, TYPE &secondary, uint64_t &primary)           \
   {                                                                                                                      \
      return priv_context.IDX.lowerbound_secondary(code, scope, table, secondary, primary);                                    \
   }                                                                                                                      \
   int db_##IDX##_upperbound(uint64_t code, uint64_t scope, uint64_t table, TYPE &secondary, uint64_t &primary)           \
   {                                                                                                                      \
      return priv_context.IDX.upperbound_secondary(code, scope, table, secondary, primary);                                    \
   }                                                                                                                      \
   int db_##IDX##_end(uint64_t code, uint64_t scope, uint64_t table)                                                      \
   {                                                                                                                      \
      return priv_context.IDX.end_secondary(code, scope, table);                                                               \
   }                                                                                                                      \
   int db_##IDX##_next(int iterator, uint64_t &primary)                                                                   \
   {                                                                                                                      \
      return priv_context.IDX.next_secondary(iterator, primary);                                                               \
   }                                                                                                                      \
   int db_##IDX##_previous(int iterator, uint64_t &primary)                                                               \
   {                                                                                                                      \
      return priv_context.IDX.previous_secondary(iterator, primary);                                                           \
   }
...
#define DB_API_METHOD_WRAPPERS_ARRAY_SECONDARY(IDX, ARR_SIZE, ARR_ELEMENT_TYPE)                                                                             \
   ...
   {                                                                                                                                                        \
      ...
      return priv_context.IDX.store(scope, table, payer, id, data.value);                                                                                        \
   }                                                                                                                                                        \
   ...
   {                                                                                                                                                        \
      EOS_ASSERT(data_len == ARR_SIZE,                                                                                                                      \
                 ...
                 "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",                                   \
                 ("given", data_len)("expected", ARR_SIZE));                                                                                                \
      return priv_context.IDX.update(iterator, payer, data.value);                                                                                               \
   }                                                                                                                                                        \
   void db_##IDX##_remove(int iterator)                                                                                                                     \
   {                                                                                                                                                        \
      return priv_context.IDX.remove(iterator);                                                                                                                  \
   ...
   {                                                                                                                                                        \
      EOS_ASSERT(data_len == ARR_SIZE,                                                                                                                      \
                 ...
                 "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",                                   \
                 ("given", data_len)("expected", ARR_SIZE));                                                                                                \
      return priv_context.IDX.find_secondary(code, scope, table, data, primary);                                                                                 \
   }                                                                                                                                                        \
   ...
   {                                                                                                                                                        \
      EOS_ASSERT(data_len == ARR_SIZE,                                                                                                                      \
                 ...
                 "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",                                   \
                 ("given", data_len)("expected", ARR_SIZE));                                                                                                \
      return priv_context.IDX.find_primary(code, scope, table, data.value, primary);                                                                             \
   }                                                                                                                                                        \
   ...
   {                                                                                                                                                        \
      EOS_ASSERT(data_len == ARR_SIZE,                                                                                                                      \
                 ...
                 "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",                                   \
                 ("given", data_len)("expected", ARR_SIZE));                                                                                                \
      return priv_context.IDX.lowerbound_secondary(code, scope, table, data.value, primary);                                                                     \
   }                                                                                                                                                        \
   ...
   {                                                                                                                                                        \
      EOS_ASSERT(data_len == ARR_SIZE,                                                                                                                      \
                 ...
                 "invalid size of secondary key array for " #IDX ": given ${given} bytes but expected ${expected} bytes",                                   \
                 ("given", data_len)("expected", ARR_SIZE));                                                                                                \
      return priv_context.IDX.upperbound_secondary(code, scope, table, data.value, primary);                                                                     \
   }                                                                                                                                                        \
   int db_##IDX##_end(uint64_t code, uint64_t scope, uint64_t table)                                                                                        \
   {                                                                                                                                                        \
      return priv_context.IDX.end_secondary(code, scope, table);                                                                                                 \
...
#define DB_API_METHOD_WRAPPERS_FLOAT_SECONDARY(IDX, TYPE)                                                                      \
   int db_##IDX##_store(uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const TYPE &secondary)                    \
   {                                                                                                                           \
      EOS_ASSERT(!priv_softfloat_api::is_nan(secondary), transaction_exception, "NaN is not an allowed value for a secondary key"); \
      return priv_context.IDX.store(scope, table, payer, id, secondary);                                                            \
   }                                                                                                                           \
   void db_##IDX##_update(int iterator, uint64_t payer, const TYPE &secondary)                                                 \
   {                                                                                                                           \
...
class priv_database_api : public priv_context_aware_api
{
public:
   using priv_context_aware_api::priv_context_aware_api;
   ...
   {
      return priv_context.db_store_i64(scope, table, payer, id, buffer, buffer_size);
   }
   // int priv_db_store_i64( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, array_ptr<const char> buffer, size_t buffer_size ) {
   //    return priv_context.priv_db_store_i64( scope, table, payer, id, buffer, buffer_size );
   // }
   void db_update_i64(int itr, uint64_t payer, array_ptr<const char> buffer, size_t buffer_size)
   {
      priv_context.db_update_i64(itr, payer, buffer, buffer_size);
   }
   // void priv_db_update_i64( int itr, uint64_t payer, array_ptr<const char> buffer, size_t buffer_size ) {
   //    priv_context.priv_db_update_i64( itr, payer, buffer, buffer_size );
   // }
   void db_remove_i64(int itr)
   {
      priv_context.db_remove_i64(itr);
   }
   // void priv_db_remove_i64( int itr ) {
   //    priv_context.priv_db_remove_i64( itr );
   // }
   int db_get_i64(int itr, array_ptr<char> buffer, size_t buffer_size)
   {
      return priv_context.db_get_i64(itr, buffer, buffer_size);
   }
   // int priv_db_get_i64( int itr, array_ptr<char> buffer, size_t buffer_size ) {
   //    return priv_context.priv_db_get_i64(itr, buffer, buffer_size);
   // }
   int db_next_i64(int itr, uint64_t &primary)
   {
      return priv_context.db_next_i64(itr, primary);
   }
   // int priv_db_next_i64( int itr, uint64_t& primary ) {
   //    return priv_context.priv_db_next_i64(itr, primary);
   // }
   int db_previous_i64(int itr, uint64_t &primary)
   {
      return priv_context.db_previous_i64(itr, primary);
   }
   int db_find_i64(uint64_t code, uint64_t scope, uint64_t table, uint64_t id)
   {
      return priv_context.db_find_i64(code, scope, table, id);
   }
   int priv_db_find_i64( uint64_t code, uint64_t scope, uint64_t table, uint64_t id ) {
      return priv_context.db_find_i64( code, scope, table, id );
   }
   int db_lowerbound_i64(uint64_t code, uint64_t scope, uint64_t table, uint64_t id)
   {
      return priv_context.db_lowerbound_i64(code, scope, table, id);
   }
   int db_upperbound_i64(uint64_t code, uint64_t scope, uint64_t table, uint64_t id)
   {
      return priv_context.db_upperbound_i64(code, scope, table, id);
   }
   int db_end_i64(uint64_t code, uint64_t scope, uint64_t table)
   {
      return priv_context.db_end_i64(code, scope, table);
   }
...
class priv_memory_api : public priv_context_aware_api
{
public:
   priv_memory_api(priv_apply_context &ctx)
       : priv_context_aware_api(ctx, true) {}
...
class priv_transaction_api : public priv_context_aware_api
{
public:
   using priv_context_aware_api::priv_context_aware_api;
   ...
   void send_inline(array_ptr<char> data, size_t data_len)
   {
      ...
      EOS_ASSERT(data_len < priv_context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
                 "inline action too big");

      action act;
      fc::raw::unpack<action>(data, data_len, act);
      priv_context.execute_inline(std::move(act));
   }
   ...
   void send_context_free_inline(array_ptr<char> data, size_t data_len)
   {
      ...
      action act;
      fc::raw::unpack<action>(data, data_len, act);
      priv_context.execute_context_free_inline(std::move(act));
   }
   ...
   void send_deferred(const uint128_t &sender_id, account_name payer, array_ptr<char> data, size_t data_len, uint32_t replace_existing)
   {
      ...
      priv_context.schedule_deferred_transaction(sender_id, payer, std::move(trx), replace_existing);
   }

   bool cancel_deferred(const unsigned __int128 &val)
   {
      fc::uint128_t sender_id(val >> 64, uint64_t(val));
      return priv_context.cancel_deferred_transaction((unsigned __int128)sender_id);
   }
};

class priv_ccontext_free_transaction_api : public priv_context_aware_api
{
public:
   priv_ccontext_free_transaction_api(priv_apply_context &ctx)
       : priv_context_aware_api(ctx, true) {}

   int read_transaction(array_ptr<char> data, size_t buffer_size)
   {
      bytes trx = priv_context.get_packed_transaction();
   ...
   {
      return priv_context.get_packed_transaction().size();
   }

   int expiration()
   {
      return priv_context.trx_context.trx.expiration.sec_since_epoch();
   }

   int tapos_block_num()
   {
      return priv_context.trx_context.trx.ref_block_num;
   }
   int tapos_block_prefix()
   {
      return priv_context.trx_context.trx.ref_block_prefix;
   }

   int get_action(uint32_t type, uint32_t index, array_ptr<char> buffer, size_t buffer_size) const
   {
      return priv_context.get_action(type, index, buffer, buffer_size);
   }
};

class compiler_builtins : public priv_context_aware_api
{
public:
   compiler_builtins(priv_apply_context &ctx)
       : priv_context_aware_api(ctx, true) {}
   ...
   int __unordtf2(uint64_t la, uint64_t ha, uint64_t lb, uint64_t hb)
   {
      float128_t a = {{la, ha}};
      float128_t b = {{lb, hb}};
      if (priv_softfloat_api::is_nan(a) || priv_softfloat_api::is_nan(b))
         return 1;
      return 0;
   }
 ...
 */
class priv_call_depth_api : public priv_context_aware_api {
   public:
      priv_call_depth_api(priv_apply_context &ctx)
          : priv_context_aware_api(ctx, true) {}
...
// REGISTER_INJECTED_INTRINSICS(priv_call_depth_api, 
...
REGISTER_INTRINSICS(priv_privileged_api,
...
REGISTER_INJECTED_INTRINSICS(priv_transaction_context,
   (checktime,      void())
);


REGISTER_INTRINSICS(priv_producer_api,
...
REGISTER_INTRINSICS( priv_database_api,
   (db_store_i64,             int(int64_t,int64_t,int64_t,int64_t,int,int))
   //(priv_db_store_i64,        int(int64_t,int64_t,int64_t,int64_t,int,int))
   (db_update_i64,            void(int,int64_t,int,int))
   //(priv_db_update_i64,       void(int,int64_t,int,int))
   (db_remove_i64,            void(int))
   //(priv_db_remove_i64,       void(int))
   (db_get_i64,               int(int, int, int))
   //(priv_db_get_i64,          int(int, int, int))
   (db_next_i64,              int(int, int))
   //(priv_db_next_i64,         int(int, int))
   (db_previous_i64,          int(int, int))
   (db_find_i64,              int(int64_t,int64_t,int64_t,int64_t))
   //(priv_db_find_i64,         int(int64_t,int64_t,int64_t,int64_t))
...
REGISTER_INTRINSICS(priv_crypto_api,
...
REGISTER_INTRINSICS(priv_permission_api,
...
REGISTER_INTRINSICS(priv_system_api,
   (current_time, int64_t()       )
   (publication_time,   int64_t() )
);

REGISTER_INTRINSICS(priv_context_free_system_api,
   (abort,                void()              )
...
REGISTER_INTRINSICS(priv_action_api,
...
REGISTER_INTRINSICS(priv_authorization_api,
   (require_recipient,     void(int64_t)          )
   (require_authorization, void(int64_t), "require_auth", void(priv_authorization_api::*)(const account_name&) )
   (require_authorization, void(int64_t, int64_t), "require_auth2", void(priv_authorization_api::*)(const account_name&, const permission_name& permission) )
   (has_authorization,     int(int64_t), "has_auth", bool(priv_authorization_api::*)(const account_name&)const )
   (is_account,            int(int64_t)           )
);

REGISTER_INTRINSICS(priv_console_api,
   (prints,                void(int)      )
...
REGISTER_INTRINSICS(priv_ccontext_free_transaction_api,
...
REGISTER_INTRINSICS(priv_transaction_api,
...
REGISTER_INTRINSICS(priv_context_free_api,
   (get_context_free_data, int(int, int, int) )
)

REGISTER_INTRINSICS(priv_memory_api,
...
REGISTER_INJECTED_INTRINSICS(priv_softfloat_api,
...
std::istream &operator>>(std::istream &in, priv_wasm_interface::vm_type &runtime)
{
   std::string s;
   in >> s;
   if (s == "wavm")
      runtime = eosio::chain::priv_wasm_interface::vm_type::wavm;
   else if (s == "wabt")
      runtime = eosio::chain::priv_wasm_interface::vm_type::wabt;
   else
