...
namespace eosio { namespace chain {
   ...
   typedef multi_index_container<
      priv_block_state_ptr,
      indexed_by<
         hashed_unique< tag<by_block_id>, member<priv_block_header_state, block_id_type, &priv_block_header_state::id>, std::hash<block_id_type>>,
         ordered_non_unique< tag<by_prev>, const_mem_fun<priv_block_header_state, const block_id_type&, &priv_block_header_state::prev> >,
         ordered_non_unique< tag<by_block_num>,
            composite_key< priv_block_state,
               member<priv_block_header_state,uint32_t,&priv_block_header_state::block_num>,
               member<priv_block_state,bool,&priv_block_state::in_current_chain>
            >,
            composite_key_compare< std::less<uint32_t>, std::greater<bool> >
         >,
         ordered_non_unique< tag<by_lib_block_num>,
            composite_key< priv_block_header_state,
                member<priv_block_header_state,uint32_t,&priv_block_header_state::dpos_irreversible_blocknum>,
                member<priv_block_header_state,uint32_t,&priv_block_header_state::bft_irreversible_blocknum>,
                member<priv_block_header_state,uint32_t,&priv_block_header_state::block_num>
            >,
   ...
   struct priv_fork_database_impl {
   ...
   priv_fork_database::priv_fork_database( const fc::path& data_dir ):my( new priv_fork_database_impl() ) {
      my->datadir = data_dir;
      ...
      if( fc::exists( fork_db_dat ) ) {
         ...
         for( uint32_t i = 0, n = size.value; i < n; ++i ) {
            priv_block_state s;
            fc::raw::unpack( ds, s );
            set( std::make_shared<priv_block_state>( move( s ) ) );
         }
   ...
   priv_fork_database::~priv_fork_database() {
      close();
   }

   void priv_fork_database::set( priv_block_state_ptr s ) {
   ...
   priv_block_state_ptr priv_fork_database::add( const priv_block_state_ptr& n, bool skip_validate_previous ) {
      ...
      //std::cout << "           E O S I O           :    [Start] [priv_fork_database] [add] [block number]  " << n->block->block_num() << std::endl;
   ...
   priv_block_state_ptr priv_fork_database::add( priv_signed_block_ptr b, bool skip_validate_signee ) {
      ...
      auto result = std::make_shared<priv_block_state>( **prior, move(b), skip_validate_signee );
      EOS_ASSERT( result, fork_database_exception , "fail to add new block state" );
      return add(result, true);
   }

   const priv_block_state_ptr& priv_fork_database::head()const { return my->head; }

   /**
    *  Given two head blocks, return two branches of the fork graph that
    *  end with a common ancestor (same prior block)
    */
   pair< branch_type, branch_type >  priv_fork_database::fetch_branch_from( const block_id_type& first,
   ...
   void priv_fork_database::remove( const block_id_type& id ) {
   ...
   void priv_fork_database::set_validity( const priv_block_state_ptr& h, bool valid ) {
      if( !valid ) {
         remove( h->id );
      } else {
   ...
   void priv_fork_database::mark_in_current_chain( const priv_block_state_ptr& h, bool in_current_chain ) {
   ...
   void priv_fork_database::prune( const priv_block_state_ptr& h ) {
      ...
      std::cout << "           E O S I O           :    [END] [fork_database] [prune] " << std::endl;
   }

   priv_block_state_ptr   priv_fork_database::get_block(const block_id_type& id)const {
      std::cerr << "daesony : priv_fork_database get_block!!" << std::endl;
   ...
   priv_block_state_ptr   priv_fork_database::get_block_in_current_chain_by_num( uint32_t n )const {
   ...
   void priv_fork_database::add( const header_confirmation& c ) {
    ...
    */
   void priv_fork_database::set_bft_irreversible( block_id_type id ) {
