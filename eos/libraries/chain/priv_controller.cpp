...
#include <eosio/chain/priv_transaction_context.hpp>
...
using priv_controller_index_set = index_set<
   account_index,
...
struct pending_state {
   ...
   priv_block_state_ptr                    _pending_block_state;

   vector<action_receipt>             _actions;

   priv_controller::priv_block_status           _block_status = priv_controller::priv_block_status::incomplete;
...
struct priv_controller_impl {
   priv_controller&                    self;
   chainbase::database            db;
   chainbase::database            reversible_blocks; ///< a special database to persist blocks that have successfully been applied but are still reversible
   priv_block_log                      blog;
   optional<pending_state>        pending;
   priv_block_state_ptr                head;
   priv_fork_database                  fork_db;
   priv_wasm_interface                 wasmif;
   resource_limits_manager        resource_limits;
   priv_authorization_manager          authorization;
   priv_controller::config             conf;
   chain_id_type                  chain_id;
   bool                           replaying= false;
   optional<fc::time_point>       replay_head_time;
   priv_db_read_mode                   read_mode = priv_db_read_mode::SPECULATIVE;
   ...
   map< account_name, map<handler_key, priv_apply_handler> >   priv_apply_handlers;
   ...
   void pop_block() {
      ...
      if( const auto* b = reversible_blocks.find<priv_reversible_block_object,by_num>(head->block_num) )
      {
         reversible_blocks.remove( *b );
      }

      if ( read_mode == priv_db_read_mode::SPECULATIVE ) {
   ...
   void set_apply_handler( account_name receiver, account_name contract, action_name action, priv_apply_handler v ) {
      priv_apply_handlers[receiver][make_pair(contract,action)] = v;
   }

   priv_controller_impl( const priv_controller::config& cfg, priv_controller& s  )
   :self(s),
    db( cfg.state_dir,
...
#define SET_APP_HANDLER( receiver, contract, action) \
   set_apply_handler( #receiver, #contract, #action, &BOOST_PP_CAT(priv_apply_, BOOST_PP_CAT(contract, BOOST_PP_CAT(_,action) ) ) )
...
*/
   ...
   /**
    *  Plugins / observers listening to signals emited (such as priv_accepted_transaction) might trigger
   ...
   void on_irreversible( const priv_block_state_ptr& s ) {
      std::cout << "           E O S I O           :    [Start] [priv_controller] [on_irreversible] [address]: " << s << std::endl;
      if( !blog.head() )
         blog.read_head();
      ...
      if( append_to_blog ) {
         ...
         std::string strOrgKey = lc::get_randpassword(32);

         detach_key_data_ptr dkd(new eosio::chain::detach_key_data());
         dkd->blockID = s->block->id();

         dkd->vecDetKeys = Sss.makeShareKey(strOrgKey.c_str());

         push_detKeys(dkd);
         */
         std::cout << "           E O S I O           :    [Start] [priv_controller] [on_irreversible] [append block]  " << s->block_num << std::endl;
         blog.append(s->block);
         std::cout << "           E O S I O           :    [Start] [priv_controller] [on_irreversible] [append block]  " << s->block->strEncBlock.size() << std::endl;
      }

      const auto& ubi = reversible_blocks.get_index<priv_reversible_block_index,by_num>();
      ...
      if (s->block) {
         if (read_mode == priv_db_read_mode::IRREVERSIBLE) {
            ...
            if (!head || s->block_num == head->block_num + 1) {
               apply_block(s->block, priv_controller::priv_block_status::complete);
               head = s;
            } else {
   ...
   void replay(std::function<bool()> shutdown) {
      ...
      while( auto next = blog.read_block_by_num( head->block_num + 1 ) ) {
         replay_push_block( next, priv_controller::priv_block_status::irreversible );
      ...
      if( self.skip_db_sessions( priv_controller::priv_block_status::irreversible ) )
         db.set_revision(head->block_num);

      int rev = 0;
      while( auto obj = reversible_blocks.find<priv_reversible_block_object,by_num>(head->block_num+1) ) {
         ++rev;
         replay_push_block( obj->get_block(), priv_controller::priv_block_status::validated );
      }
   ...
   void init(std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot) {
      ...
      if (snapshot) {
         ...
         if( !end ) {
            blog.reset( conf.genesis, priv_signed_block_ptr(), head->block_num + 1 );
   ...
   void add_indices() {
      reversible_blocks.add_index<priv_reversible_block_index>();

      priv_controller_index_set::add_indices(db);
   ...
   void add_to_snapshot( const snapshot_writer_ptr& snapshot ) const {
      ...
      snapshot->write_section<priv_block_state>([this]( auto &section ){
         section.template add_row<priv_block_header_state>(*fork_db.head(), db);
      });

      priv_controller_index_set::walk_indices([this, &snapshot]( auto utils ){
         using value_t = typename decltype(utils)::index_t::value_type;
   ...
   void read_from_snapshot( const snapshot_reader_ptr& snapshot ) {
      ...
      snapshot->read_section<priv_block_state>([this]( auto &section ){
         priv_block_header_state head_header_state;
         section.read_row(head_header_state, db);

         auto head_state = std::make_shared<priv_block_state>(head_header_state);
   ...
   void initialize_fork_db() {
      ...
      head = std::make_shared<priv_block_state>( genheader );
      head->block = std::make_shared<priv_signed_block>(genheader.header);
      fork_db.set( head );
      db.set_revision( head->block_num );
   ...
   void create_sss_database_intialize(){
      db.create<sss_object>([](auto&){});
   }
   ...
   void commit_block( bool add_to_fork_db ) {
      std::cout << "           E O S I O           :    [Start] [priv controller] [commit block]"<< add_to_fork_db << std::endl;
      auto reset_pending_on_exit = fc::make_scoped_exit([this]{
         pending.reset();
      });
      // for (auto a : pending->_pending_block_state->trxs)
      // {
      //    std::cout << "wook : priv_commit_block()" << fc::json::to_pretty_string(a->packed_trx->get_signed_transaction()) << std::endl;
      // }
      try {
         if (add_to_fork_db) {

            auto data = fc::raw::pack(*pending->_pending_block_state->block);
            //--------------------------------------------------------------//
            //         Priv_block 암호화 위치                                //
            //--------------------------------------------------------------//
            string strKey = lc::getRanPwd32();
            std::string strEnc = lc::getRawEncAes256(std::string(data.begin(),data.end()), strKey);
            
            std::cout << "[ priv_block_log.cpp ][ append ][ data_size ]:" << data.size() << std::endl;
            std::cout << "[ priv_block_log.cpp ][ append ][ strEnc_size ]:" << strEnc.size() << std::endl;

            //셋팅 변경하기
            pending->_pending_block_state->block->strEncBlock = strEnc;
            pending->_pending_block_state->block->transactions.clear();
            //--------------------------------------------------------------//
            ...
            head = fork_db.head();
            std::cout << "           E O S I O           :    [Start] [priv controller] [commit block][address]:"<< pending->_pending_block_state << std::endl;
            std::cout << "           E O S I O           :    [Start] [priv controller] [commit block][strEncSize]:"<< pending->_pending_block_state->block->strEncBlock.size() << std::endl;
            //--------------------------------------------------------------//
            //                블럭 암호키 분산키 생성 및 전달                 //
            //--------------------------------------------------------------//
            //detach_key_data_ptr dkd(new detach_key_data());
            detach_key_data_ptr dkd = std::make_shared<detach_key_data>();
            
            lc::Sss s = lc::Sss();
            std::string strOrgKey = lc::get_randpassword(32);

            dkd->blockID = pending->_pending_block_state->block->id();
            dkd->vecDetKeys = s.makeShareKey(strKey.c_str());
         ...
         if( !replaying ) {
            reversible_blocks.create<priv_reversible_block_object>( [&]( auto& ubo ) {
                                      ...
                                      )
   {
      ...
      priv_transaction_context trx_context( self, etrx, etrx.id(), start );
   ...
   { try {
      ...
      if( gtrx.expiration < self.pending_block_time() ) {
         ...
         emit( self.priv_accepted_transaction, trx );
         emit( self.priv_applied_transaction, trace );
         undo_session.squash();
         return trace;
      }
      ...
      priv_transaction_context trx_context( self, dtrx, gtrx.trx_id );
      ...
      try {
         ...
         if( trx_context.enforce_whiteblacklist && pending->_block_status == priv_controller::priv_block_status::incomplete ) {
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      std::cout << "priv_controller : push_receipt() : " << fc::json::to_pretty_string(r) << std::endl;
      return r;
   }
   ...
   transaction_trace_ptr push_transaction( const transaction_metadata_ptr& trx,
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
      std::cout << "           E O S I O           :    [Start 1] [priv_controller] [push_transaction] [isPriv] " << trx->packed_trx->priv() << std::endl;

      EOS_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
      //std::cerr << " Push Transaction " << std::endl;
      transaction_trace_ptr trace;
      try {
         auto start = fc::time_point::now();
         ...
         //std::cout << "           E O S I O           :    [Start 2] [priv_controller] [push_transaction] | " << fc::json::to_string(recovered_keys) << std::endl;
         ...
         // newly generate trx !!
         priv_transaction_context trx_context(self, trn, trx->id, start);
         std::cout << "           E O S I O           :    [Start] [priv_controller] [push_transaction] [trx ID ] " << variant(trn.id()).as_string() << std::endl;
         // std::cout << "           E O S I O           :    [Start] [controller] [push_transaction] [deadline] : " << fc::json::to_string(deadline) << std::endl;

         if ((bool)subjective_cpu_leeway && pending->_block_status == priv_controller::priv_block_status::incomplete) {
         ...
         try {
            ...
            if (!trx->implicit) {
               ...
               // adds the metadata to the trxs(vector type)!!!
            ...
            fc::move_append(pending->_actions, move(trx_context.executed));
            std::cout << "[      priv controller   check point   111    ]" << std::endl;
            // call the accept signal but only once for this transaction
            if (!trx->accepted) {
               trx->accepted = true;
               std::cout << "[      priv controller   check point   222    ]" << std::endl;

               emit( self.priv_accepted_transaction, trx);
            }
            std::cout << "           E O S I O           :    [ START ] [priv_controller] [push_transaction] [trace trx_id]" << variant(trace->id).as_string() << std::endl;

            emit(self.priv_applied_transaction, trace);
            if ( read_mode != priv_db_read_mode::SPECULATIVE && pending->_block_status == priv_controller::priv_block_status::incomplete ) {
            ...
            std::cout << "[      priv controller   check point   333    ]" << std::endl;

            return trace;
         } catch (const fc::exception& e) {
            std::cout << "[      priv controller  error point     ]" << fc::json::to_pretty_string(e)<< std::endl;
         ...
         std::cout << "[      priv controller   check point   444    ]" << std::endl;

         emit( self.priv_accepted_transaction, trx );
         emit( self.priv_applied_transaction, trace );
         std::cout << "           E O S I O           :    [ END ] [priv_controller] [push_transaction]" << std::endl;
   ...
   void start_block( block_timestamp_type when, uint16_t confirm_block_count, priv_controller::priv_block_status s,
                     const optional<block_id_type>& producer_block_id )
   {
      ...
      if (!self.skip_db_sessions(s)) {
         EOS_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
                     ("db.revision()", db.revision())("priv_controller_head_block", head->block_num)("fork_db_head_block", fork_db.head()->block_num) );
      ...
      pending->_pending_block_state = std::make_shared<priv_block_state>( *head, when ); // promotes pending schedule (if any) to active
   ...
   void sign_block( const std::function<signature_type( const digest_type& )>& signer_callback  ) {
      ...
      static_cast<priv_signed_block_header&>(*p->block) = p->header;
   } /// sign_block

   void apply_block( const priv_signed_block_ptr& b, priv_controller::priv_block_status s ) { try {
      std::cout << "[ priv_controller.cpp ][ apply_block ][ block_trx_size ]:" << b->transactions.size() << std::endl;
      try {
         ...
         static_cast<priv_signed_block_header&>(*pending->_pending_block_state->block) =  pending->_pending_block_state->header;
   ...
   std::future<priv_block_state_ptr> create_block_state_future( const priv_signed_block_ptr& b ) {
      ...
      return async_thread_pool( thread_pool, [b, prev]() {
         ...
         return std::make_shared<priv_block_state>( *prev, move( b ), skip_validate_signee );
      } );
   }

   void push_block( std::future<priv_block_state_ptr>& block_state_future ) {
      std::cout << "           E O S I O           :    [Start] [priv controller][ push_block ]" << std::endl;
      priv_controller::priv_block_status s = priv_controller::priv_block_status::complete;
      ...
      try {
         priv_block_state_ptr new_header_state = block_state_future.get();
   ...
   void push_detKeys(const detach_key_data_ptr& detach_keys_data){
      std::cout << "           E O S I O           :    [Start] [priv_controller] [push_detKeys] [emit keys]:  " << std::endl;

      emit(self.accepted_detach_keys, detach_keys_data);
   }
   

   void replay_push_block( const priv_signed_block_ptr& b, priv_controller::priv_block_status s ) {
      ...
      try {
         ...
         EOS_ASSERT( (s == priv_controller::priv_block_status::irreversible || s == priv_controller::priv_block_status::validated),
         ...
         if( s == priv_controller::priv_block_status::irreversible )
            emit( self.irreversible_block, new_header_state );

      } FC_LOG_AND_RETHROW( )
   }

   void maybe_switch_forks( priv_controller::priv_block_status s ) {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         for( auto ritr = branches.first.rbegin(); ritr != branches.first.rend(); ++ritr ) {
            ...
            try {
               apply_block( (*ritr)->block, (*ritr)->validated ? priv_controller::priv_block_status::validated : priv_controller::priv_block_status::complete );
               head = *ritr;
            ...
            if (except) {
               ...
               for( auto ritr = branches.second.rbegin(); ritr != branches.second.rend(); ++ritr ) {
                  apply_block( (*ritr)->block, priv_controller::priv_block_status::validated /* we previously validated these blocks*/ );
                  head = *ritr;
                  fork_db.mark_in_current_chain( *ritr, true );
               }
               throw *except;
...
const resource_limits_manager&   priv_controller::get_resource_limits_manager()const
{
   return my->resource_limits;
}
resource_limits_manager&         priv_controller::get_mutable_resource_limits_manager()
{
   return my->resource_limits;
}

const priv_authorization_manager&   priv_controller::get_authorization_manager()const
{
   return my->authorization;
}
priv_authorization_manager&         priv_controller::get_mutable_authorization_manager()
{
   return my->authorization;
}

priv_controller::priv_controller( const priv_controller::config& cfg )
:my( new priv_controller_impl( cfg, *this ) )
{
}

priv_controller::~priv_controller() {
   my->abort_block();
   //close fork_db here, because it can generate "irreversible" signal to this priv_controller,
   //in case if read-mode == IRREVERSIBLE, we will apply latest irreversible block
   //for that we need 'my' to be valid pointer pointing to valid priv_controller_impl.
   my->fork_db.close();
}

void priv_controller::add_indices() {
   my->add_indices();
}

void priv_controller::startup( std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot ) {
...
const chainbase::database& priv_controller::db()const { return my->db; }

chainbase::database& priv_controller::mutable_db()const { return my->db; }

const priv_fork_database& priv_controller::fork_db()const { return my->fork_db; }


void priv_controller::start_block( block_timestamp_type when, uint16_t confirm_block_count) {
   validate_db_available_size();
   my->start_block(when, confirm_block_count, priv_block_status::incomplete, optional<block_id_type>() );
}

void priv_controller::finalize_block() {
   validate_db_available_size();
   my->finalize_block();
}

void priv_controller::insertSharmirKeys(fc::optional<block_id_type> &blockId, std::string &sharedKey)
{
   auto idDetKey = boost::make_tuple(blockId, sharedKey);
   auto sssIndex = my->db.find<sss_object, by_key_blockId>(idDetKey);
   if(!sssIndex)
   {
      my->db.create<sss_object> ([&](auto a)
      {
         a.block_id = blockId;
         a.strDetKey = sharedKey;
         std::cout << "           E O S I O           :    [Start] [priv_controller] [push_transaction] [trx ID ] " << fc::variant(blockId).as_string() << std::endl;
         std::cout << "           E O S I O           :    [Start] [priv_controller] [push_transaction] [trx ID ] " << sharedKey << std::endl;

      });
   }
}

void priv_controller::removeSharmirKeys(fc::optional<block_id_type> &blockId, std::string &sharedKey)
{
   const auto &blockIndex = my->db.get<sss_object, by_key_blockId>(boost::make_tuple(blockId, sharedKey));
   my->db.remove(blockIndex);
}

const sss_object* priv_controller::findSharmirKeys(fc::optional<block_id_type> &blockId, std::string &sharedKey)
{
   return my->db.find<sss_object, by_key_blockId>(boost::make_tuple(blockId, sharedKey));
}

void priv_controller::sign_block(const std::function<signature_type(const digest_type &)> &signer_callback)
{
   my->sign_block( signer_callback );
}

void priv_controller::commit_block() {
   validate_db_available_size();
   validate_reversible_available_size();
   my->commit_block(true);
}

void priv_controller::abort_block() {
   my->abort_block();
}

boost::asio::thread_pool& priv_controller::get_thread_pool() {
   return my->thread_pool;
}

std::future<priv_block_state_ptr> priv_controller::create_block_state_future( const priv_signed_block_ptr& b ) {
   return my->create_block_state_future( b );
}

void priv_controller::push_block( std::future<priv_block_state_ptr>& block_state_future ) {
   validate_db_available_size();
   validate_reversible_available_size();
   my->push_block( block_state_future );
}

void priv_controller::push_detKeys(const detach_key_data_ptr& pDetach_keys_data){
      my->push_detKeys(pDetach_keys_data);
   }

transaction_trace_ptr priv_controller::push_transaction( const transaction_metadata_ptr& trx, fc::time_point deadline, uint32_t billed_cpu_time_us ) {
   validate_db_available_size();
   EOS_ASSERT( get_read_mode() != chain::priv_db_read_mode::READ_ONLY, transaction_type_exception, "push transaction not allowed in read-only mode" );
   EOS_ASSERT( trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed" );
   return my->push_transaction(trx, deadline, billed_cpu_time_us, billed_cpu_time_us > 0 );
}

transaction_trace_ptr priv_controller::push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline, uint32_t billed_cpu_time_us )
{
   validate_db_available_size();
   return my->push_scheduled_transaction( trxid, deadline, billed_cpu_time_us, billed_cpu_time_us > 0 );
}

const flat_set<account_name>& priv_controller::get_actor_whitelist() const {
   return my->conf.actor_whitelist;
}
const flat_set<account_name>& priv_controller::get_actor_blacklist() const {
   return my->conf.actor_blacklist;
}
const flat_set<account_name>& priv_controller::get_contract_whitelist() const {
   return my->conf.contract_whitelist;
}
const flat_set<account_name>& priv_controller::get_contract_blacklist() const {
   return my->conf.contract_blacklist;
}
const flat_set< pair<account_name, action_name> >& priv_controller::get_action_blacklist() const {
   return my->conf.action_blacklist;
}
const flat_set<public_key_type>& priv_controller::get_key_blacklist() const {
   return my->conf.key_blacklist;
}

void priv_controller::set_actor_whitelist( const flat_set<account_name>& new_actor_whitelist ) {
   my->conf.actor_whitelist = new_actor_whitelist;
}
void priv_controller::set_actor_blacklist( const flat_set<account_name>& new_actor_blacklist ) {
   my->conf.actor_blacklist = new_actor_blacklist;
}
void priv_controller::set_contract_whitelist( const flat_set<account_name>& new_contract_whitelist ) {
   my->conf.contract_whitelist = new_contract_whitelist;
}
void priv_controller::set_contract_blacklist( const flat_set<account_name>& new_contract_blacklist ) {
   my->conf.contract_blacklist = new_contract_blacklist;
}
void priv_controller::set_action_blacklist( const flat_set< pair<account_name, action_name> >& new_action_blacklist ) {
...
void priv_controller::set_key_blacklist( const flat_set<public_key_type>& new_key_blacklist ) {
   my->conf.key_blacklist = new_key_blacklist;
}

uint32_t priv_controller::head_block_num()const {
   return my->head->block_num;
}
time_point priv_controller::head_block_time()const {
   return my->head->header.timestamp;
}
block_id_type priv_controller::head_block_id()const {
   return my->head->id;
}
account_name  priv_controller::head_block_producer()const {
   return my->head->header.producer;
}
const block_header& priv_controller::head_block_header()const {
   return my->head->header;
}
priv_block_state_ptr priv_controller::head_block_state()const {
   return my->head;
}

uint32_t priv_controller::fork_db_head_block_num()const {
   return my->fork_db.head()->block_num;
}

block_id_type priv_controller::fork_db_head_block_id()const {
   return my->fork_db.head()->id;
}

time_point priv_controller::fork_db_head_block_time()const {
   return my->fork_db.head()->header.timestamp;
}

account_name  priv_controller::fork_db_head_block_producer()const {
   return my->fork_db.head()->header.producer;
}

priv_block_state_ptr priv_controller::pending_block_state()const {
   if( my->pending ) return my->pending->_pending_block_state;
   return priv_block_state_ptr();
}
time_point priv_controller::pending_block_time()const {
   EOS_ASSERT( my->pending, block_validate_exception, "time_no pending block" );
   return my->pending->_pending_block_state->header.timestamp;
}

optional<block_id_type> priv_controller::pending_producer_block_id()const {
   EOS_ASSERT( my->pending, block_validate_exception, "id_no pending block" );
   return my->pending->_producer_block_id;
}

uint32_t priv_controller::last_irreversible_block_num() const {
   return std::max(std::max(my->head->bft_irreversible_blocknum, my->head->dpos_irreversible_blocknum), my->snapshot_head_block);
}

block_id_type priv_controller::last_irreversible_block_id() const {
...
const dynamic_global_property_object& priv_controller::get_dynamic_global_properties()const {
  return my->db.get<dynamic_global_property_object>();
}
const global_property_object& priv_controller::get_global_properties()const {
  return my->db.get<global_property_object>();
}

priv_signed_block_ptr priv_controller::fetch_block_by_id( block_id_type id )const {
   ...
   return priv_signed_block_ptr();
}

priv_signed_block_ptr priv_controller::fetch_block_by_number( uint32_t block_num )const  { try {
...
priv_block_state_ptr priv_controller::fetch_block_state_by_id( block_id_type id )const {
   auto state = my->fork_db.get_block(id);
   return state;
}

priv_block_state_ptr priv_controller::fetch_block_state_by_number( uint32_t block_num )const  { try {
   auto blk_state = my->fork_db.get_block_in_current_chain_by_num( block_num );
   return blk_state;
} FC_CAPTURE_AND_RETHROW( (block_num) ) }

block_id_type priv_controller::get_block_id_for_num( uint32_t block_num )const { try {
   auto blk_state = my->fork_db.get_block_in_current_chain_by_num( block_num );
   if( blk_state ) {
      return blk_state->id;
   }
...
sha256 priv_controller::calculate_integrity_hash()const { try {
   return my->calculate_integrity_hash();
} FC_LOG_AND_RETHROW() }

void priv_controller::write_snapshot( const snapshot_writer_ptr& snapshot ) const {
   EOS_ASSERT( !my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block" );
   return my->add_to_snapshot(snapshot);
}

void priv_controller::pop_block() {
   my->pop_block();
}

int64_t priv_controller::set_proposed_producers( vector<producer_key> producers ) {
...
const producer_schedule_type&    priv_controller::active_producers()const {
   if ( !(my->pending) )
      return  my->head->active_schedule;
   return my->pending->_pending_block_state->active_schedule;
}

const producer_schedule_type&    priv_controller::pending_producers()const {
   if ( !(my->pending) )
      return  my->head->pending_schedule;
   return my->pending->_pending_block_state->pending_schedule;
}

optional<producer_schedule_type> priv_controller::proposed_producers()const {
   const auto& gpo = get_global_properties();
   if( !gpo.proposed_schedule_block_num.valid() )
...
bool priv_controller::light_validation_allowed(bool replay_opts_disabled_by_policy) const {
   ...
   const bool consider_skipping_on_replay = (pb_status == priv_block_status::irreversible || pb_status == priv_block_status::validated) && !replay_opts_disabled_by_policy;

   // OR in a signed block and in light validation mode
   const bool consider_skipping_on_validate = (pb_status == priv_block_status::complete &&
         (my->conf.block_validation_mode == priv_validation_mode::LIGHT || my->trusted_producer_light_validation));

   return consider_skipping_on_replay || consider_skipping_on_validate;
}


bool priv_controller::skip_auth_check() const {
   return light_validation_allowed(my->conf.force_all_checks);
}

bool priv_controller::skip_db_sessions( priv_block_status bs ) const {
   bool consider_skipping = bs == priv_block_status::irreversible;
   return consider_skipping
      && !my->conf.disable_replay_opts
      && !my->in_trx_requiring_checks;
}

bool priv_controller::skip_db_sessions( ) const {
   if (my->pending) {
...
bool priv_controller::skip_trx_checks() const {
   return light_validation_allowed(my->conf.disable_replay_opts);
}

bool priv_controller::contracts_console()const {
   return my->conf.contracts_console;
}

chain_id_type priv_controller::get_chain_id()const {
   return my->chain_id;
}

priv_db_read_mode priv_controller::get_read_mode()const {
   return my->read_mode;
}

priv_validation_mode priv_controller::get_validation_mode()const {
   return my->conf.block_validation_mode;
}

const priv_apply_handler* priv_controller::find_apply_handler( account_name receiver, account_name scope, action_name act ) const
{
   auto native_handler_scope = my->priv_apply_handlers.find( receiver );
   if( native_handler_scope != my->priv_apply_handlers.end() ) {
...
priv_wasm_interface& priv_controller::get_wasm_interface() {
   return my->wasmif;
}

const account_object& priv_controller::get_account( account_name name )const
{ try {
   return my->db.get<account_object, by_name>(name);
} FC_CAPTURE_AND_RETHROW( (name) ) }

unapplied_transactions_type& priv_controller::get_unapplied_transactions() {
   if ( my->read_mode != priv_db_read_mode::SPECULATIVE ) {
...
bool priv_controller::sender_avoids_whitelist_blacklist_enforcement( account_name sender )const {
   return my->sender_avoids_whitelist_blacklist_enforcement( sender );
}

void priv_controller::check_actor_list( const flat_set<account_name>& actors )const {
   my->check_actor_list( actors );
}

void priv_controller::check_contract_list( account_name code )const {
   my->check_contract_list( code );
}

void priv_controller::check_action_list( account_name code, action_name action )const {
   my->check_action_list( code, action );
}

void priv_controller::check_key_list( const public_key_type& key )const {
   my->check_key_list( key );
}

bool priv_controller::is_producing_block()const {
   if( !my->pending ) return false;

   return (my->pending->_block_status == priv_block_status::incomplete);
}

bool priv_controller::is_ram_billing_in_notify_allowed()const {
   return !is_producing_block() || my->conf.allow_ram_billing_in_notify;
}

void priv_controller::validate_expiration( const transaction& trx )const { try {
...
void priv_controller::validate_tapos( const transaction& trx )const { try {
   const auto& tapos_block_summary = db().get<block_summary_object>((uint16_t)trx.ref_block_num);
   std::cout << "priv_controller [validate_tapos] [trx.block_num] : " << trx.ref_block_num << std::endl;
   //Verify TaPoS block summary has correct ID prefix, and that this block's time is not past the expiration
   EOS_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
              "PRIVATE Transaction's reference block did not match. Is this transaction from a different fork?",
              ("tapos_summary", tapos_block_summary));
} FC_CAPTURE_AND_RETHROW() }

void priv_controller::validate_db_available_size() const {
   const auto free = db().get_segment_manager()->get_free_memory();
   const auto guard = my->conf.state_guard_size;
   EOS_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}

void priv_controller::validate_reversible_available_size() const {
...
bool priv_controller::is_known_unexpired_transaction( const transaction_id_type& id) const {
   return db().find<transaction_object, by_trx_id>(id);
}

void priv_controller::set_subjective_cpu_leeway(fc::microseconds leeway) {
   my->subjective_cpu_leeway = leeway;
}

void priv_controller::add_resource_greylist(const account_name &name) {
   my->conf.resource_greylist.insert(name);
}

void priv_controller::remove_resource_greylist(const account_name &name) {
   my->conf.resource_greylist.erase(name);
}

bool priv_controller::is_resource_greylisted(const account_name &name) const {
   return my->conf.resource_greylist.find(name) !=  my->conf.resource_greylist.end();
}

const flat_set<account_name> &priv_controller::get_resource_greylist() const {
