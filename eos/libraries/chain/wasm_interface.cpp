...
namespace eosio { namespace chain {
   ...
   void wasm_interface::validate(const controller& control, const bytes& code) {
      ...
      std::cout << "           E O S I O           :    [Start] [wasm_interface.cpp] [validation][ TRUE ] " << std::endl;
    ...
    {
       std::cout << "           E O S I O           :    [Start] [wasm_interface] [apply]:" << code.size() << std::endl;
       my->get_instantiated_module(code_id, code, context.trx_context)->apply(context); // wavm.cpp -> apply(){ call("apply", args, context);}
   }
...
class action_api : public context_aware_api {
   ...
   action_api( apply_context& ctx )
      ...
      int action_data_size() {
         std::cout << "action_data_size" << context.act.data.size() << std::endl;
...
class database_api : public context_aware_api {
   public:
      ...
      int priv_db_get_i64( int itr, array_ptr<char> buffer, size_t buffer_size ) {
         return context.priv_db_get_i64(itr, buffer, buffer_size);
      }
      int db_next_i64( int itr, uint64_t& primary ) {
         return context.db_next_i64(itr, primary);
      }
      int priv_db_next_i64( int itr, uint64_t& primary ) {
         return context.priv_db_next_i64(itr, primary);
      }
