...
{
   //std::cout << "           E O S I O           :    [Start] [apply_contrext.cpp] [exec_one]  " << std::endl;
   auto start = fc::time_point::now();

   action_receipt r;
   r.receiver         = receiver;
   r.act_digest       = digest_type::hash(act);
   //std::cout << "wook: apply_context.cpp : exec_one() act JSON "<< fc::json::to_pretty_string(act) <<std::endl;
   ...
   try {
      try {
         ...
         //std::cout << "           E O S I O           :    [Start] [apply_context] [exec_one] [receiver account object] " << fc::json::to_pretty_string(a) << std::endl;
         privileged = a.privileged;
         //std::cout << "           E O S I O           :    [Start] [apply_context] [exec_one] [receiver] " << receiver << std::endl;
         //std::cout << "           E O S I O           :    [Start] [apply_context] [exec_one] [act.account] " << act.account << std::endl;
         //std::cout << "           E O S I O           :    [Start] [apply_context] [exec_one] [act.name] " << act.name << std::endl;
         ...
         if( a.code.size() > 0
            ...
            try {
               std::cout << "           E O S I O           :    [Start] [apply_context] [exec_one] [a.code_version] AFTER " << fc::json::to_string(a.code_version) << std::endl;
               std::cout << "           E O S I O           :    [Start] [apply_context] [exec_one] [a.code.size()] AFTER" << a.code.size() << std::endl;
               std::cout << "           E O S I O           :    [Start] [apply_context] [exec_one] [a.abi.size()] AFTER" << a.abi.size() << std::endl;
               // contract executed
   ...
   //std::cout << "           E O S I O           :    [END] [apply_context] [exec_one]  " << std::endl;
}
...
const table_id_object* apply_context::priv_find_table( name code, name scope, name table ) {
   return db.find<table_id_object, by_code_scope_table>(boost::make_tuple(code, scope, table));
}
...
const table_id_object& apply_context::priv_find_or_create_table( name code, name scope, name table, const account_name &payer ) {
   std::cout <<"wook : priv_find_or_create_table" << std::endl;
   const auto* existing_tid =  db.find<table_id_object, by_code_scope_table>(boost::make_tuple(code, scope, table));
   if (existing_tid != nullptr) {
      return *existing_tid;
   }
...
// void apply_context::priv_remove_table( const table_id_object& tid ) {
//    update_db_usage(tid.payer, - config::billable_size_v<table_id_object>);
//    priv_db.remove(tid);
// }
...
//   require_write_lock( scope );
   ...
   auto tableid = tab.id;
   std::cout << "           E O S I O           :    [Start] [apply_context] [db_store_i64] [payer] " << payer << std::endl;
   std::cout << "           E O S I O           :    [Start] [apply_context] [db_store_i64] [buffer] " << buffer << std::endl;
   ...
   std::cout << "           E O S I O           :    [END] [apply_context] [db_store_i64] " << buffer << std::endl;
...
//   require_write_lock( table_obj.scope );
   std::cout << "           E O S I O           :    [Start] [apply_context] [db_update_i64] [payer] " << payer << std::endl;
   std::cout << "           E O S I O           :    [Start] [apply_context] [db_update_i64] [buffer] " << buffer << std::endl;
   ...
   std::cout << "           E O S I O           :    [END] [apply_context] [db_update_i64] " << std::endl;
}
...
int apply_context::priv_db_store_i64( uint64_t scope, uint64_t table, const account_name& payer, uint64_t id, const char* buffer, size_t buffer_size ) {
   return priv_db_store_i64( receiver, scope, table, payer, id, buffer, buffer_size);
}

int apply_context::priv_db_store_i64( uint64_t code, uint64_t scope, uint64_t table, const account_name& payer, uint64_t id, const char* buffer, size_t buffer_size ) {
//   require_write_lock( scope );
   const auto& tab = find_or_create_table( code, scope, table, payer );
   auto tableid = tab.id;
   EOS_ASSERT( payer != account_name(), invalid_table_payer, "must specify a valid account to pay for new record" );
   ...
   int64_t billable_size = (int64_t)(buffer_size + config::billable_size_v<key_value_object>);
   update_db_usage( payer, billable_size);
   // change the iterator_cache
   keyval_cache.cache_table( tab );
   return keyval_cache.add( obj );
}

void apply_context::priv_db_update_i64( int iterator, account_name payer, const char* buffer, size_t buffer_size ) {
   std::cout << "wook : apply_context.cpp : priv_db_update_i64() " << std::endl;
   const key_value_object& obj = keyval_cache.get( iterator );
   const auto& table_obj = keyval_cache.get_table( obj.t_id );
   EOS_ASSERT( table_obj.code == receiver, table_access_violation, "db access violation" );
...
void apply_context::priv_db_remove_i64( int iterator ) {
   std::cout << "wook : apply_context.cpp : priv_db_remove_i64() " << std::endl;
   const key_value_object& obj = keyval_cache.get( iterator );
...
int apply_context::priv_db_get_i64( int iterator, char* buffer, size_t buffer_size ) {
   const key_value_object& obj = keyval_cache.get( iterator );
   auto s = obj.value.size();
   if( buffer_size == 0 ) return s;
   auto copy_size = std::min( buffer_size, s );
...
int apply_context::priv_db_next_i64( int iterator, uint64_t& primary ) {
   if( iterator < -1 ) return -1; // cannot increment past end iterator of table
...
int apply_context::priv_db_find_i64( uint64_t code, uint64_t scope, uint64_t table, uint64_t id ) {
   //require_read_lock( code, scope ); // redundant?
   const auto* tab = priv_find_table( code, scope, table );
   if( !tab ) return -1;
