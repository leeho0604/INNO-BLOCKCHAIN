...
namespace Runtime
{
	...
	bool IntrinsicResolver::resolve(const std::string& moduleName,const std::string& exportName,ObjectType type,ObjectInstance*& outObject)
	{
		...
		//std::cout << "IntrinsicResolver::resolve, moduleName & exportName : " << moduleName << " : " << exportName << std::endl;
	...
	bool IntrinsicResolver::resolvePriv(const std::string &moduleName, const std::string &exportName, ObjectType type, ObjectInstance *&outObject)
	{
		// Make sure the wavmIntrinsics module can't be directly imported.
		...
		//std::cout << "IntrinsicResolver::resolvePriv, moduleName & exportName : " << moduleName << " : " << exportName << std::endl;
		outObject = Intrinsics::privFind(moduleName + "." + exportName, type);
		return outObject != nullptr;
	}

	template <typename Instance, typename Type>
	void privLinkImport(const IR::Module &module, const Import<Type> &import, Resolver &resolver, LinkResult &linkResult, std::vector<Instance *> &resolvedImports)
	{
		// Ask the resolver for a value for this import.
		ObjectInstance *importValue;
		if (resolver.resolve(import.moduleName, import.exportName, resolveImportType(module, import.type), importValue))
		{
			//std::cout << "privLinkImport()" << std::endl;
			// Sanity check that the resolver returned an object of the right type.
			WAVM_ASSERT_THROW(isA(importValue, resolveImportType(module, import.type)));
			resolvedImports.push_back(as<Instance>(importValue));
		}
	...
	LinkResult privLinkModule(const IR::Module &module, Resolver &resolver)
	{
		LinkResult linkResult;
		for (const auto &import : module.functions.imports)
		{
			privLinkImport(module, import, resolver, linkResult, linkResult.resolvedImports.functions);
		}
		for (const auto &import : module.tables.imports)
		{
			privLinkImport(module, import, resolver, linkResult, linkResult.resolvedImports.tables);
		}
		for (const auto &import : module.memories.imports)
		{
			privLinkImport(module, import, resolver, linkResult, linkResult.resolvedImports.memories);
		}
		for (const auto &import : module.globals.imports)
		{
			privLinkImport(module, import, resolver, linkResult, linkResult.resolvedImports.globals);
		}
