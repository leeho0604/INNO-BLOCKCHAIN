...
namespace Intrinsics
{
	...
	struct FunctionPR
	{
		...
		RUNTIME_API FunctionPR(const char *inName, const IR::FunctionType *type, void *nativeFunction);
		RUNTIME_API ~FunctionPR();
	...
	struct GlobalPR
	{
		...
		RUNTIME_API GlobalPR(const char *inName, IR::GlobalType inType);
		RUNTIME_API ~GlobalPR();
	...
	template <IR::ValueType type, bool isMutable>
	struct GenericGlobalPR : GlobalPR
	{
		typedef typename IR::ValueTypeInfo<type>::Value Value;

		GenericGlobalPR(const char *inName, Value inValue)
			: GlobalPR(inName, IR::GlobalType(type, isMutable)) { *(Value *)value = inValue; }

		operator Value() const { return *(Value *)value; }
		void operator=(Value newValue) { *(Value *)value = newValue; }
	};
	template <IR::ValueType type>
	struct GenericGlobalPR<type, false> : GlobalPR
	{
		typedef typename IR::ValueTypeInfo<type>::Value Value;

		GenericGlobalPR(const char *inName, Value inValue)
			: GlobalPR(inName, IR::GlobalType(type, false)) { *(Value *)value = inValue; }
	...
	struct MemoryPR
	{
		RUNTIME_API MemoryPR(const char *inName, const IR::MemoryType &inType);
		RUNTIME_API ~MemoryPR();
	...
	struct TablePR
	{
		RUNTIME_API TablePR(const char *inName, const IR::TableType &inType);
		RUNTIME_API ~TablePR();
	...
	// Finds an intrinsic object by name and type.
	RUNTIME_API Runtime::ObjectInstance *privFind(const std::string &name, const IR::ObjectType &type);
