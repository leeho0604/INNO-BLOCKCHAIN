...
namespace Runtime
{
	...
	struct Resolver
	{
		//virtual bool resolvePriv(const std::string &moduleName, const std::string &exportName, IR::ObjectType type, ObjectInstance *&outObject) = 0;
	...
	struct IntrinsicResolver : Resolver
	{
		...
		RUNTIME_API bool resolvePriv(const std::string &moduleName, const std::string &exportName, IR::ObjectType type, ObjectInstance *&outObject);
	};
	...
	RUNTIME_API LinkResult privLinkModule(const IR::Module &module, Resolver &resolver);
}