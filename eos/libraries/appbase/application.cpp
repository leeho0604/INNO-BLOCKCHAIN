...
class application_impl {
   public:
      ...
      bfs::path               _priv_data_dir{"priv-data-dir"};
...
void application::set_default_priv_data_dir(const bfs::path& priv_data_dir) {
  my->_priv_data_dir = priv_data_dir;
}
...
void application::startup() {
   ...
   try {
      std::cout << "ghlee : plugin set Start---" << std::endl;
      for( auto plugin : initialized_plugins ) {
         if( is_quiting() ) break;
         plugin->startup();
      }
      std::cout << "ghlee : plugin set End---" << std::endl;
   } catch( ... ) {
...
{
   std::cout << "ghlee set_plugin_cnt:" << plugins.size() << std::endl;
   for(auto& plug : plugins) {
      std::cout << "ghlee : set_plugins = " << plug.second->name() << std::endl;
   ...
   app_cli_opts.add_options()
         ...
         ("priv-data-dir,d", bpo::value<std::string>(), "Directory containing program runtime priv data")
...
bool application::initialize_impl(int argc, char** argv, vector<abstract_plugin*> autostart_plugins) {
   ...
   std::cerr << "daesony : application::initialize_impl() " << std::endl;
   ...
   if( options.count( "priv-data-dir" ) ) {
      // Workaround for 10+ year old Boost defect
      // See https://svn.boost.org/trac10/ticket/8535
      // Should be .as<bfs::path>() but paths with escaped spaces break bpo e.g.
      // std::exception::what: the argument ('/path/with/white\ space') for option '--data-dir' is invalid
      auto workaround = options["priv-data-dir"].as<std::string>();
      bfs::path priv_data_dir = workaround;
      if( priv_data_dir.is_relative() )
         priv_data_dir = bfs::current_path() / priv_data_dir;
      my->_priv_data_dir = priv_data_dir;
   }
   ...
   if(options.count("plugin") > 0)
   {
	  ...
	  std::cerr << "daesony : application::initialize_impl() : plugin cnt : " << options.count("plugin") << std::endl;
      ...
      for(auto& arg : plugins)
      {
         ...
         for(const std::string& name : names){
            std::cerr << "daesony : application::initialize_impl() plugin name : " << name << std::endl;
   ...
   try {
      ...
      {
         std::cerr << "daesony : application::initialize_impl() : abstract_plugin : " << plugin->name() << std::endl;
...
abstract_plugin* application::find_plugin(const string& name)const
{
   ...
   std::cout<< "app_find_plugin:"<<name <<std::endl;
   if(itr == plugins.end()) {
      return nullptr;
   }
...
bfs::path application::priv_data_dir() const {
   return my->_priv_data_dir;
}
