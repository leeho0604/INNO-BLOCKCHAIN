...
Export* Module::GetExport(string_view name) {
  ...
  //std::cout << "Module::GetExport : field_index " << field_index << std::endl;
...
Index Module::AppendExport(ExternalKind kind,
  ...
  ////std::cout << "export_->name:" << name.data() << std::endl;
  //std::cout << "OnExport : name , item_index, exports.size : " << export_->name << " : " << item_index << " : " << exports.size()-1 << std::endl;
...
Result Thread::CallHost(HostFunc* func) {
  ...
  //std::cout << "Thread::CallHost [START]" << std::endl;
  ////std::cout << "CallHost] [num_results]" << num_params << std::endl;
  TypedValues params(num_params);
  TypedValues results(num_results);

  ////std::cout << "CallHost] [POINT_1]" << num_params << std::endl;
  for (size_t i = num_params; i > 0; --i) {
    params[i - 1].value = Pop();
    params[i - 1].type = sig->param_types[i - 1];
  }
  ////std::cout << "CallHost] [POINT_2]" << num_params << std::endl;
  for (size_t i = 0; i < num_results; ++i) {
    results[i].type = sig->result_types[i];
    results[i].SetZero();
  }
  // if (func->field_name.compare("checktime") != 0){
  //   //std::cout << "CallHost] [STRAT]" << func->module_name << " | "
  //             //<< func->field_name << "/" << num_params << "/" << num_results
  //             //<< std::endl;
  // }
  Result call_result = func->callback(func, sig, params, results);
  if (func->field_name.compare("checktime") != 0) {
    //std::cout << "CallHost] [AFTER]" << num_params << std::endl;
  }
    ...
    // //std::cout << "CallHost] [END]" << std::endl;
    return Result::Ok;
}
  ...
  for (int i = 0; i < num_instructions; ++i) {
    ...
    switch (opcode) {
      ...
      case Opcode::Call: {
        ...
        //std::cout << "CallHost : Opcode::Call " << std::endl;
...
ExecResult Executor::RunFunction(Index func_index, const TypedValues& args) {
  //std::cout << "           E O S I O           :    [Start] [interp.cc] [RunFunction] [func_index] || " << func_index << std::endl;
  ExecResult exec_result;
  Func* func = env_->GetFunc(func_index);
  //std::cout << "RunFunction_POINT_2" << std::endl;
  FuncSignature* sig = env_->GetFuncSignature(func->sig_index);
   // std::cout << "           E O S I O           :    [Start] [interp.cc] [RunFunction] [func_index] || " << func->sig_index << std::endl;
  //std::cout << "RunFunction_POINT_3" << std::endl;
  thread_.Reset();
  usleep(4);
  //std::cout << "RunFunction_POINT_4" << std::endl;
  exec_result.result = PushArgs(sig, args);
  //std::cout << "RunFunction_POINT_5" << std::endl;
  if (exec_result.result == Result::Ok) {
    //std::cout << "RunFunction_POINT_6" << std::endl;
    exec_result.result =
        func->is_host ? thread_.CallHost(cast<HostFunc>(func))
                      : RunDefinedFunction(cast<DefinedFunc>(func)->offset);
    if (exec_result.result == Result::Ok) {
      //std::cout << "RunFunction_POINT_7" << std::endl;
      CopyResults(sig, &exec_result.values);
    }
  }
  //std::cout << "RunFunction_POINT_8" << std::endl;
  //std::cout << "RunFunction_POINT_8 exec_result " << exec_result.result() << std::endl;
  return exec_result;
}
...
ExecResult Executor::RunExportByName(Module* module,
  ...
  //Export* export_2 = module->GetExport("priv_db_store_i64");
  if (!export_) {
  ...
  // }
  // else{
  //   std::cout << "           E O S I O           :    [Start] [interp.cc] [RunExportByName] [priv_db_store_i64] || true"<< std::endl;
  // }
  if (export_->kind != ExternalKind::Func) {
...
Result Executor::RunDefinedFunction(IstreamOffset function_offset) {
  //std::cout << "RunDefinedFunction [START]" << std::endl;
  //std::cout << "RunDefinedFunction Offset : " << function_offset << std::endl;
  Result result = Result::Ok;
  thread_.set_pc(function_offset);
  //std::cout << "RunDefinedFunction [POINT_1]" << std::endl;
  if (trace_stream_) {
    //std::cout << "RunDefinedFunction [POINT_2]" << std::endl;
    ...
    //std::cout << "RunDefinedFunction [POINT_3]" << std::endl;
  ...
  if (result != Result::Returned) {
    //std::cout << "RunDefinedFunction [ERROR]" << std::endl;
    return result;
  }
  // Use OK instead of RETURNED for consistency.
  //std::cout << "RunDefinedFunction [END]" << std::endl;
  return Result::Ok;
}
