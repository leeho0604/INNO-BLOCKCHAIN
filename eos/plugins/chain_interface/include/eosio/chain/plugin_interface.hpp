...
namespace eosio { namespace chain { namespace plugin_interface {
   ...
   namespace priv_channels {
      using pre_accepted_block     = channel_decl<struct pre_accepted_block_tag,    priv_signed_block_ptr>;
      using rejected_block         = channel_decl<struct rejected_block_tag,        priv_signed_block_ptr>;
      using accepted_block_header  = channel_decl<struct accepted_block_header_tag, priv_block_state_ptr>;
      using accepted_block         = channel_decl<struct accepted_block_tag,        priv_block_state_ptr>;
      using irreversible_block     = channel_decl<struct irreversible_block_tag,    priv_block_state_ptr>;
   ...
   namespace priv_methods {
      using get_block_by_number    = method_decl<chain_plugin_interface, priv_signed_block_ptr(uint32_t block_num)>;
      using get_block_by_id        = method_decl<chain_plugin_interface, priv_signed_block_ptr(const block_id_type& block_id)>;
   ...
   namespace incoming {
      ...
      namespace priv_channels {
         using block                 = channel_decl<struct block_tag, priv_signed_block_ptr>;
         using transaction           = channel_decl<struct transaction_tag, transaction_metadata_ptr>;
      }
      ...
      namespace priv_methods {
         // synchronously push a block/trx to a single provider
         using block_sync            = method_decl<chain_plugin_interface, void(const priv_signed_block_ptr&), first_provider_policy>;
         using transaction_async     = method_decl<chain_plugin_interface, void(const transaction_metadata_ptr&, bool, next_function<transaction_trace_ptr>), first_provider_policy>;
      }
   }
