...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   std::cerr << "daesony : chain_plugin::plugin_initialize()" << std::endl;
   ...
   try {
      ...
      if( options.count( "priv-blocks-dir" )) {
         ...
         if( bld.is_relative()){
            my->priv_blocks_dir = app().priv_data_dir() / bld;
         } else{
            my->priv_blocks_dir = bld;
         }
      }
      ...
      my->chain_config->priv_blocks_dir         = my->priv_blocks_dir;
      my->chain_config->priv_state_dir          = app().priv_data_dir() / config::default_priv_state_dir_name;
      ...
      ilog("daesony : chain_plugin thread_pool_size : "+ my->chain_config->thread_pool_size);
      ...
      // ** chain emplace a chain config ** 
      my->chain.emplace( *my->chain_config );
      // Private chain id create??
      ...
      my->accepted_transaction_connection = my->chain->accepted_transaction.connect(
            [this]( const transaction_metadata_ptr& meta ) {
             std::cout << "           E O S I O           :    [ SIGNAL ] [chain_plugin] [accepted_transaction] [ priv ] : "  << meta->packed_trx->priv()<< std::endl;
             std::cout << "           E O S I O           :    [ SIGNAL ] [chain_plugin] [accepted_transaction] [ id ] :" << variant(meta->packed_trx->id()).as_string() << std::endl;
             my->accepted_transaction_channel.publish(priority::low, meta);
            } );

      my->applied_transaction_connection = my->chain->applied_transaction.connect(
            [this]( const transaction_trace_ptr& trace ) {
               std::cout << "           E O S I O           :    [ SIGNAL ] [chain_plugin] [applied_transaction]" << variant(trace->id).as_string() << std::endl;
...
// reversible_blocks to another node
...
read_only::get_info_results read_only::get_info(const read_only::get_info_params&) const {
   ...
   std::cerr << "daesony : chain_plugin read_only::get_info() : " << std::endl;
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   std::cout << "chain_plugin : get_block() : " << fc::json::to_pretty_string(pretty_output) << std::endl;
...
void read_write::push_block(read_write::push_block_params&& params, next_function<read_write::push_block_results> next) {
   try {
      std::cout << "[chain_plugin][ push_block ]" << std::endl;
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   try {
      ...
      try {
         ...
         //std::cout << "           E O S I O           :    [Start] [chain_plugin] [push_transaction] [isPrive]  |  " << ptrx->packed_trx->priv() << std::endl;
