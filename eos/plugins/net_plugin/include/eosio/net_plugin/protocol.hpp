...
namespace eosio {
   ...
   struct handshake_message {
      ...
      string                     agent;
      int16_t                    generation;
      uint                       unType; // Chain Message Check
   };
  ...
  struct notice_message {
    ...
    uint unType; // 1=Public Block, 2=Private Block
  };
  ...
  struct request_message {
   ...
   struct skey_request_message{
      skey_request_message () : publicKey() {}
      std::string publicKey;
   };

   struct key_registry_message{
      key_registry_message () : scKey() {}
      string  scKey;
   };

   struct encryt_message{
      unsigned int unType;
      std::string strEnc;

      encryt_message(const unsigned int& un, const std::string& str):unType(un),strEnc(str){}
      encryt_message():unType(),strEnc(){}
   };
   // unType 별 사용 내역
   
   // [ 0 ]: BP 확인 후 의사난수 전송(O: 의사난수 전달 / X: 의사난수가 아닌 0값 전달)
   // [ 1 ]: 의사난수 수집 및 분할키 전송
   // [ 2 ]: 분할키 값 전달
   // [ 3 ]: 분할키 값 수집
   /*
   struct ordered_data{
      ...
      std::string                   strDetKey;
   };
   */
   /*
   struct ordered_message{
      // 1: BP여부 및 의사난수 요청 / 0: BP여부 및 의사난수 전달
      bool                          isReq;
      fc::optional<block_id_type>   block_id;
   ...
   struct detached_key_message{
      fc::optional<block_id_type>   block_id;
      std::string strDetKey;

      detached_key_message() : block_id(), strDetKey(){}
      detached_key_message(const fc::optional<block_id_type>& bid, const std::string& str) : block_id(bid), strDetKey(str){}
   };
   */
   using net_message = static_variant<handshake_message,
                                      ...
                                      priv_signed_block,       // which = 9
                                      skey_request_message,    // which = 10
                                      key_registry_message,    // which = 11
                                      encryt_message>;         // which = 12
                                    //   ordered_message,    // which = 12
                                    //   detached_key_message>;   // which = 13
...
FC_REFLECT( eosio::handshake_message,
            ...
            (last_irreversible_block_num)(last_irreversible_block_id)
            (head_num)(head_id)
            (os)(agent)(generation)(unType) )
FC_REFLECT( eosio::go_away_message, (reason)(node_id) )
FC_REFLECT( eosio::time_message, (org)(rec)(xmt)(dst) )
FC_REFLECT( eosio::notice_message, (known_trx)(known_blocks)(unType) )
FC_REFLECT( eosio::request_message, (req_trx)(req_blocks)(unType) )
FC_REFLECT( eosio::sync_request_message, (start_block)(end_block)(unType) )

FC_REFLECT( eosio::skey_request_message, (publicKey) )
FC_REFLECT( eosio::key_registry_message, (scKey) )
FC_REFLECT( eosio::encryt_message, (unType)(strEnc) )

// FC_REFLECT( eosio::ordered_message, (isReq)(block_id)(unRndNumber)(strIP) )
// FC_REFLECT( eosio::detached_key_message, (block_id)(strDetKey) )
/**
 *
