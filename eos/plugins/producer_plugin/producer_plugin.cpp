...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      optional<fc::time_point> calculate_next_priv_block_time(const account_name& producer_name, const block_timestamp_type& current_block_time) const;
      void schedule_production_loop();
      void schedule_production_priv_loop();
      void produce_block();
      void produce_priv_block();
      bool maybe_produce_block();
      bool maybe_produce_priv_block();

      boost::program_options::variables_map _options;
      bool     _production_enabled                 = false;
      bool     _priv_production_enabled                 = false;
      bool     _pause_production                   = false;
      bool     _priv_pause_production                   = false;
      ...
      fc::microseconds                                          _max_irreversible_priv_block_age_us;
      ...
      fc::time_point                                            _irreversible_priv_block_time;
      fc::microseconds                                          _keosd_provider_timeout_us;

      time_point _last_signed_block_time;
      time_point _last_priv_signed_block_time;
      time_point _start_time = fc::time_point::now();
      time_point _priv_start_time = fc::time_point::now();
      uint32_t   _last_signed_block_num = 0;
      uint32_t   _last_priv_signed_block_num = 0;
      ...
      //new
      incoming::priv_channels::block::channel_type::handle         _incoming_priv_block_subscription;
      //incoming::priv_channels::transaction::channel_type::handle   _incoming_priv_transaction_subscription;

      incoming::priv_methods::block_sync::method_type::handle        _incoming_priv_block_sync_provider;
      //incoming::priv_methods::transaction_async::method_type::handle _incoming_priv_transaction_async_provider;

      transaction_id_with_expiry_index                         _blacklisted_priv_transactions;

      fc::optional<scoped_connection>                          _accepted_priv_block_connection;
      fc::optional<scoped_connection>                          _irreversible_priv_block_connection;
      /*
       * HACK ALERT
      ...
      uint32_t _priv_timer_corelation_id = 0;
      ...
      /*
      bool bp_check(account_name name){
         if(!_producer_watermarks[my->_producers])
            return false;

         return true;
      }*/
      // NEW
      void on_priv_block( const priv_block_state_ptr& bsp ) {
         if( bsp->header.timestamp <= _last_priv_signed_block_time ) return;
         if( bsp->header.timestamp <= _priv_start_time ) return;
         if( bsp->block_num <= _last_priv_signed_block_num ) return;
            ...
            if( producer != bsp->header.producer ) {
               ...
               if( itr != active_producer_to_signing_key.end() ) {
                  ...
                  if( private_key_itr != _signature_providers.end() ) {
                     ...
                     _last_priv_signed_block_time = bsp->header.timestamp;
                     _last_priv_signed_block_num  = bsp->block_num;
   ...
   //                  ilog( "${n} confirmed", ("n",name(producer)) );
         ...
         chain::priv_controller& chain = priv_chain_plug->chain();
   ...
   //                  ilog( "${n} confirmed", ("n",name(producer)) );
      ...
      void on_irreversible_priv_block( const priv_signed_block_ptr& lib ) {
         _irreversible_priv_block_time = lib->timestamp.to_time_point();
      }
      ...
      void on_incoming_block(const signed_block_ptr& block) {
         auto id = block->id();
         std::cout << "           E O S I O           :    [Start] [producer_plugin] [on_incoming_block][ START ]" << std::endl;
         ...
         */
         if (block->timestamp > (fc::time_point::now() + fc::seconds(7))){
            std::cout << "           E O S I O           :    [Start] [producer_plguin] [on_incoming_block] [TIMEOVER]" << std::endl;
         }
         chain::controller& chain = chain_plug->chain();
         // chain::controller& chain = priv_chain_plug->chain();
         ...
         bool except = false;
         try {
            std::cout << "           E O S I O           :    [Start] [producer_plugin] [on_incoming_block][ PUSH_BLOCK ]" << std::endl;
         ...
         std::cout << "           E O S I O           :    [Start] [producer_plugin] [on_incoming_block][ END ]" << std::endl;
      ...
      void on_incoming_priv_block(const priv_signed_block_ptr& block) {
         auto id = block->id();
         //std::cout << "           E O S I O           :    [Start] [producer_plugin] [on_incoming_priv_block]" << std::endl;
         fc_dlog(_log, "received incoming priv_block ${id}", ("id", id));
         /*
         EOS_ASSERT( block->timestamp < (fc::time_point::now() + fc::seconds( 7 )), block_from_the_future,
                     "received a block from the future, ignoring it: ${id}", ("id", id) );
                     */
         if (block->timestamp > (fc::time_point::now() + fc::seconds(7)))
         {
            std::cout << "           E O S I O           :    [Start] [producer_plguin] [on_incoming_priv_block] [TIMEOVER]" << std::endl;
         }
         chain::priv_controller &chain = priv_chain_plug->chain();
         ...
         if( except ) {
            app().get_channel<priv_channels::rejected_block>().publish( priority::medium, block );
            return;
         }
         if( chain.head_block_state()->header.timestamp.next().to_time_point() >= fc::time_point::now() ) {
            _priv_production_enabled = true;
         }

            //----------------------------------------------------//
            /*
            //Test
            if(block->block_num() % 100 == 0){
               detach_key_data_ptr dkd(new detach_key_data());
                     lc::Sss s = lc::Sss();
                     std::string strOrgKey = lc::get_randpassword(32);

                     dkd->blockID = block->id();
                     dkd->vecDetKeys = s.makeShareKey(strOrgKey.c_str());
                     
                     chain.push_detKeys(dkd);
            }
            */
            //-------------------------------------------------------//
      ...
      void on_incoming_priv_transaction_async(const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
         chain::priv_controller& chain = priv_chain_plug->chain();
         std::cout << "           E O S I O           :    [Start] [producer_plguin] [on_incoming_priv_transaction_async] [isPrive]  |  " << trx->packed_trx->priv() << std::endl;
         EOS_ASSERT(trx->packed_trx->priv() != false, transaction_type_exception, "Transation flag is not the correct");
         ...
         boost::asio::post( *_thread_pool, [self = this, future, trx, persist_until_expired, next]() {
            ...
            app().post(priority::low, [self, trx, persist_until_expired, next]() {
               self->process_incoming_priv_transaction_async(trx, persist_until_expired, next);
            });
         });
      }
      ...
      void on_incoming_transaction_async(const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
         chain::controller& chain = chain_plug->chain();
         std::cout << "           E O S I O           :    [Start] [producer_plguin] [on_incoming_transaction_async] [isPrive]  |  " << trx->packed_trx->priv() << std::endl;
         EOS_ASSERT(trx->packed_trx->priv() != true, transaction_type_exception, "Transation flag is not the correct");
         const auto &cfg = chain.get_global_properties().configuration;
         signing_keys_future_type future = transaction_metadata::start_recover_keys( trx, *_thread_pool,
               chain.get_chain_id(), fc::microseconds( cfg.max_transaction_cpu_usage ) );
         boost::asio::post( *_thread_pool, [self = this, future, trx, persist_until_expired, next]() {
      ...
      void process_incoming_transaction_async(const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
         chain::controller& chain = chain_plug->chain();
         std::cout << "           E O S I O           :    [Start] [producer_plguin] [process_incoming_transaction_async] [isPrive]  |  " << trx->packed_trx->priv() << std::endl;
         ...
         std::cout << "           E O S I O           :    [Start] [producer_plguin] [process_incoming_transaction_async] [deadline]  |  " << fc::json::to_string(deadline) << std::endl;
         ...
         try {
            ...
            if (trace->except) {
               ...
               //std::cout << "wook : producer_plugin.cpp : trace " << fc::json::to_pretty_string(*trace) << std::endl;
               send_response(trace);
            }
      ...
      void process_incoming_priv_transaction_async(const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
         chain::priv_controller& chain = priv_chain_plug->chain();
         std::cout << "           E O S I O           :    [Start] [producer_plguin] [process_incoming_priv_transaction_async] [isPrive]  |  " << trx->packed_trx->priv() << std::endl;
         if (!chain.pending_block_state())
         {
            _pending_incoming_transactions.emplace_back(trx, persist_until_expired, next);
            return;
         ...
         auto send_response = [this, &trx, &chain, &next](const fc::static_variant<fc::exception_ptr, transaction_trace_ptr>& response) {            
            ...
            if (response.contains<fc::exception_ptr>()) {
               _transaction_ack_channel.publish(priority::low, std::pair<fc::exception_ptr, transaction_metadata_ptr>(response.get<fc::exception_ptr>(), trx));
               if (_pending_priv_block_mode == pending_block_mode::producing)
               {
                  fc_dlog(_trx_trace_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is REJECTING tx: ${txid} : ${why} ",
                        ("block_num", chain.head_block_num() + 1)
                        ("prod", chain.pending_block_state()->header.producer)
         ...
         auto deadline = fc::time_point::now() + fc::milliseconds(_max_transaction_time_ms) + fc::milliseconds(_max_transaction_time_ms) + fc::milliseconds(_max_transaction_time_ms);
         std::cout << "           E O S I O           :    [Start] [producer_plguin] [process_incoming_priv_transaction_async] [deadline] 111 |  " << fc::json::to_string(deadline) << std::endl;
         bool deadline_is_subjective = false;
         //deadline += fc::milliseconds(6000000);
         //std::cout << "           E O S I O           :    [Start] [producer_plguin] [process_incoming_priv_transaction_async] [deadline] 222 |  " << fc::json::to_string(deadline) << std::endl;

         const auto block_deadline = calculate_block_deadline(block_time);
         if (_max_transaction_time_ms < 0 || (_pending_priv_block_mode == pending_block_mode::producing && block_deadline < deadline))
         {
            deadline_is_subjective = true;
            deadline = block_deadline;
         }
         ...
         try {
            std::cout << "           E O S I O           :    [Start] [producer_plguin] [process_incoming_priv_transaction_async] [encryption  1] |  " << std::endl;
            //------------------------------------//
            /*    암호키 생성해서 trx내부 암호화    */
            //------------------------------------//
            //std::string strOrgKey = lc::get_randpassword(32);
            //vector<bytes> vecByte = trx->packed_trx->get_context_free_data();
            //std::string strOrg(vecByte.begin(), vecByte.end());

            std::cout << "           E O S I O           :    [Start] [producer_plguin] [process_incoming_priv_transaction_async] [encryption  2] |  " << std::endl;
            //------------------------------------//
            auto trace = chain.push_transaction(trx, deadline);
            if (trace->except) {
               if (failure_is_subjective(*trace->except, deadline_is_subjective)) {
            ...
            /*
            lc::Sss s = lc::Sss();
            //std::vector<std::string>vecDetKeys = s.makeShareKey(strOrgKey.c_str());
            
            //
             * Get Random bytes. It would be inserted in HMAC
             * Random string is made by hmac_sha256
             * Finaly The Key were shared for everyone
             * 
            //

            detach_key_data_ptr dkd(new eosio::chain::detach_key_data());
            dkd->blockID = trace->producer_block_id;

            fc::hmac_sha256 mac;
            fc::sha256 mac_256_key = mac.digest(strOrgKey.c_str(),strOrgKey.length(), strOrgKey.c_str(), strOrgKey.length());
            std::cout << "           E O S I O           :    [Start] [producer_plguin] [process_incoming_priv_transaction_async] [hmac_256 key] :  " << mac_256_key.str() << std::endl;
            dkd->vecDetKeys = s.makeShareKey(mac_256_key.data());

            // dkd->vecDetKeys = s.makeShareKey(strOrgKey.data());
            ...
            */
            ...
            detach_key_data_ptr dkd(new eosio::chain::detach_key_data());
            dkd->blockID = trace->producer_block_id;
            dkd->vecDetKeys.push_back("11111");
            dkd->vecDetKeys.push_back("22222");
            dkd->vecDetKeys.push_back("33333");
            dkd->vecDetKeys.push_back("44444");
            dkd->vecDetKeys.push_back("55555");
      ...
      fc::microseconds get_irreversible_priv_block_age() {
         auto now = fc::time_point::now();
         if (now < _irreversible_priv_block_time) {
            return fc::microseconds(0);
         } else {
            return now - _irreversible_priv_block_time;
         }
      }
      ...
      bool production_disabled_by_priv_policy() {
         return !_priv_production_enabled || _priv_pause_production || (_max_irreversible_priv_block_age_us.count() >= 0 && get_irreversible_priv_block_age() >= _max_irreversible_priv_block_age_us);
      }
      ...
      start_block_result start_priv_block();

      fc::time_point calculate_pending_block_time() const;
      fc::time_point calculate_pending_priv_block_time() const;
      ...
      void schedule_delayed_production_priv_loop(const std::weak_ptr<producer_plugin_impl>& weak_this, const block_timestamp_type& current_block_time);
};

void new_priv_chain_banner(const eosio::chain::priv_controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ("enable-stale-production,e", boost::program_options::bool_switch()->notifier([this](bool e){my->_production_enabled = e;my->_priv_production_enabled = e;}), "Enable block production, even if the chain is stale.")
         ("pause-on-startup,x", boost::program_options::bool_switch()->notifier([this](bool p){my->_pause_production = p;my->_priv_pause_production = p;}), "Start this node in a state where production is paused")
...
chain::public_key_type producer_plugin::get_producer_key() const{
   if(my->_signature_providers.empty()){
      return chain::public_key_type();
   }
   return my->_signature_providers.begin()->first;
}
...
account_name producer_plugin::get_producer_name(){
...
{ try {
   ...
   EOS_ASSERT( my->priv_chain_plug, plugin_config_exception, "priv_chain_plugin not found" );
   my->_options = &options;
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            std::cout << "[ producer_plugin ] / pulbicKey:" << fc::variant(pubkey).as_string() << std::endl; 
   ...
   my->_max_irreversible_priv_block_age_us = fc::seconds(options.at("max-irreversible-block-age").as<int32_t>());
   ...
   my->_incoming_priv_block_subscription = app().get_channel<incoming::priv_channels::block>().subscribe([this](const priv_signed_block_ptr& priv_block){
      try {
         my->on_incoming_priv_block(priv_block);
      } FC_LOG_AND_DROP();
   });
   my->_incoming_transaction_subscription = app().get_channel<incoming::channels::transaction>().subscribe([this](const transaction_metadata_ptr& trx){
      try {
         if(trx->packed_trx->priv() == 0){
            my->on_incoming_transaction_async(trx, false, [](const auto&){});
         }
         else{
            my->on_incoming_priv_transaction_async(trx, false, [](const auto &) {});
         }
   ...
   my->_incoming_priv_block_sync_provider = app().get_method<incoming::priv_methods::block_sync>().register_provider([this](const priv_signed_block_ptr& priv_block){
      my->on_incoming_priv_block(priv_block);
   });
   ...
   my->_incoming_transaction_async_provider = app().get_method<incoming::methods::transaction_async>().register_provider([this](const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) -> void {
      if(trx->packed_trx->priv() == 0){
         ...
         my->on_incoming_priv_transaction_async(trx, persist_until_expired, next);
      }
   });
...
{ try {
   ...
   chain::priv_controller& priv_chain = my->priv_chain_plug->chain();
   EOS_ASSERT( my->_producers.empty() || priv_chain.get_read_mode() == chain::priv_db_read_mode::SPECULATIVE, plugin_config_exception,
              "node cannot have any producer-name configured because block production is impossible when priv_read_mode is not \"speculative\"" );

   EOS_ASSERT( my->_producers.empty() || priv_chain.get_validation_mode() == chain::priv_validation_mode::FULL, plugin_config_exception,
              "node cannot have any producer-name configured because block production is not safe when priv_validation_mode is not \"full\"" );

   my->_accepted_block_connection.emplace(chain.accepted_block.connect( [this]( const auto& bsp ){ my->on_block( bsp ); } ));
   my->_irreversible_block_connection.emplace(chain.irreversible_block.connect( [this]( const auto& bsp ){ my->on_irreversible_block( bsp->block ); } ));

   my->_accepted_priv_block_connection.emplace(priv_chain.accepted_block.connect( [this]( const auto& bsp ){ my->on_priv_block( bsp ); } ));
   my->_irreversible_priv_block_connection.emplace(priv_chain.irreversible_block.connect( [this]( const auto& bsp ){ my->on_irreversible_priv_block( bsp->block ); } ));
   ...
   const auto priv_lib_num = priv_chain.last_irreversible_block_num();
   const auto priv_lib = priv_chain.fetch_block_by_number(lib_num);

   if (priv_lib) {
      my->on_irreversible_priv_block(priv_lib);
   } else {
      my->_irreversible_priv_block_time = fc::time_point::maximum();
   }
   ...
   if (!my->_producers.empty()) {
      ...
      if (my->_production_enabled) {
         if (priv_chain.head_block_num() == 0) {
            new_priv_chain_banner(priv_chain);
         }
         //_production_skip_flags |= eosio::chain::skip_undo_history_check;
      }
   }

   //ilog("producer plugin:  plugin_startup() setting_end");

   my->schedule_production_loop();

   my->schedule_production_priv_loop();
...
void producer_plugin::plugin_shutdown() {
   ...
   my->_accepted_priv_block_connection.reset();
...
void producer_plugin::pause() {
   ...
   my->_priv_pause_production = true;
}

void producer_plugin::resume() {
   my->_pause_production = false;
   my->_priv_pause_production = false;
   ...
   if (my->_pending_priv_block_mode == pending_block_mode::speculating) {
      chain::priv_controller& chain = my->priv_chain_plug->chain();
      chain.abort_block();
      my->schedule_production_priv_loop();
   }
}
...
void producer_plugin::update_runtime_options(const runtime_options& options) {
   ...
   if (options.max_irreversible_block_age) {
      ...
      //my->_max_irreversible_priv_block_age_us =  fc::seconds(*options.max_irreversible_block_age);
...
optional<fc::time_point> producer_plugin_impl::calculate_next_priv_block_time(const account_name& producer_name, const block_timestamp_type& current_block_time) const {
   chain::priv_controller& chain = priv_chain_plug->chain();
...
fc::time_point producer_plugin_impl::calculate_pending_priv_block_time() const {
   const chain::priv_controller& chain = priv_chain_plug->chain();
...
producer_plugin_impl::start_block_result producer_plugin_impl::start_priv_block() {
   chain::priv_controller& chain = priv_chain_plug->chain();

   if( chain.get_read_mode() == chain::priv_db_read_mode::READ_ONLY ){
   ...
   const fc::time_point now = fc::time_point::now();
   const fc::time_point block_time = calculate_pending_priv_block_time();

   _pending_priv_block_mode = pending_block_mode::producing;
   ...
   auto irreversible_block_age = get_irreversible_priv_block_age();

   // If the next block production opportunity is in the present or future, we're synced.
   if( !_priv_production_enabled ) {
      _pending_priv_block_mode = pending_block_mode::speculating;
   } else if( _producers.find(scheduled_producer.producer_name) == _producers.end()) {
      _pending_priv_block_mode = pending_block_mode::speculating;
   } else if (signature_provider_itr == _signature_providers.end()) {
      elog("Not producing block because I don't have the private key for ${scheduled_key}", ("scheduled_key", scheduled_producer.block_signing_key));
      _pending_priv_block_mode = pending_block_mode::speculating;
   } else if ( _priv_pause_production ) {
   ...
   if (_pending_priv_block_mode == pending_block_mode::speculating) {
   ...
   if (pbs) {
      ...
      if (_pending_priv_block_mode == pending_block_mode::producing && pbs->block_signing_key != scheduled_producer.block_signing_key) {
               ...
               while( itr != unapplied_trxs.end() ) {
                     ...
                     (category == tx_category::UNEXPIRED_UNPERSISTED && _producers.empty()))
                  {
                     ...
                     continue;
                            ...
                            (category == tx_category::UNEXPIRED_UNPERSISTED && _pending_priv_block_mode == pending_block_mode::producing))
                  {
                     ++num_processed;
                     ...
                     try {
                        auto deadline = fc::time_point::now() + fc::milliseconds(_max_transaction_time_ms);
                        bool deadline_is_subjective = false;
                        if (_max_transaction_time_ms < 0 || (_pending_priv_block_mode == pending_block_mode::producing && preprocess_deadline < deadline)) {
                           deadline_is_subjective = true;
         ...
         if (_pending_priv_block_mode == pending_block_mode::producing) {
            ...
            while( sch_itr != sch_idx.end() ) {
               ...
               while (_incoming_trx_weight >= 1.0 && orig_pending_txn_size && _pending_incoming_transactions.size()) {
                  ...
                  process_incoming_priv_transaction_async(std::get<0>(e), std::get<1>(e), std::get<2>(e));
               }
               ...
               try {
                  auto deadline = fc::time_point::now() + fc::milliseconds(_max_transaction_time_ms);
                  bool deadline_is_subjective = false;
                  if (_max_transaction_time_ms < 0 || (_pending_priv_block_mode == pending_block_mode::producing && scheduled_trx_deadline < deadline)) {
                     deadline_is_subjective = true;
...
void producer_plugin_impl::schedule_production_priv_loop() {
   chain::priv_controller& chain = priv_chain_plug->chain();
   _priv_timer.cancel();
   std::weak_ptr<producer_plugin_impl> weak_this = shared_from_this();

   auto result = start_priv_block();
   switch(result){
      case start_block_result::failed:
      break;
      case start_block_result::succeeded:
      break;
      case start_block_result::waiting:
      break;
   ...
   if (result == start_block_result::failed) {
      elog("Failed to start a pending priv block, will try again later");
      _priv_timer.expires_from_now( boost::posix_time::microseconds( config::block_interval_us  / 10 ));

      // we failed to start a block, so try again later?
      _priv_timer.async_wait( app().get_priority_queue().wrap( priority::high,
          [weak_this, cid = ++_priv_timer_corelation_id]( const boost::system::error_code& ec ) {
             auto self = weak_this.lock();
             if( self && ec != boost::asio::error::operation_aborted && cid == self->_priv_timer_corelation_id ) {
                self->schedule_production_priv_loop();
             }
          } ) );
   } else if (result == start_block_result::waiting){
      if (!_producers.empty() && !production_disabled_by_priv_policy()) {
         fc_dlog(_log, "Waiting till another priv_block is received and scheduling Speculative/Production Change");
         schedule_delayed_production_priv_loop(weak_this, calculate_pending_priv_block_time());
      } else {
         fc_dlog(_log, "Waiting till another priv_block is received");
         // nothing to do until more blocks arrive
      }

   } else if (_pending_priv_block_mode == pending_block_mode::producing) {
      // we succeeded but block may be exhausted
      static const boost::posix_time::ptime epoch(boost::gregorian::date(1970, 1, 1));
      auto deadline = calculate_block_deadline(chain.pending_block_time());
      if (deadline > fc::time_point::now()) {
         // ship this block off no later than its deadline
         EOS_ASSERT( chain.pending_block_state(), missing_pending_block_state, "producing without pending_priv_block_state, start_priv_block succeeded" );
         _priv_timer.expires_at( epoch + boost::posix_time::microseconds( deadline.time_since_epoch().count() ));
         fc_dlog(_log, "Scheduling priv_Block Production on Normal Block #${num} for ${time}",
                       ("num", chain.pending_block_state()->block_num)("time",deadline));
      } else {
         EOS_ASSERT( chain.pending_block_state(), missing_pending_block_state, "producing without pending_priv_block_state" );
         ...
         if (fc::time_point::now() >= expect_time) {
            _priv_timer.expires_from_now( boost::posix_time::microseconds( 0 ));
            fc_dlog(_log, "Scheduling priv_Block Production on Exhausted priv_Block #${num} immediately",
                          ("num", chain.pending_block_state()->block_num));
         } else {
            _priv_timer.expires_at(epoch + boost::posix_time::microseconds(expect_time.time_since_epoch().count()));
            fc_dlog(_log, "Scheduling priv_Block Production on Exhausted priv_Block #${num} at ${time}",
                          ("num", chain.pending_block_state()->block_num)("time",expect_time));
         }
      }
      _priv_timer.async_wait( app().get_priority_queue().wrap( priority::high,
            [&chain,weak_this,cid=++_priv_timer_corelation_id](const boost::system::error_code& ec) {
               auto self = weak_this.lock();
               if( self && ec != boost::asio::error::operation_aborted && cid == self->_priv_timer_corelation_id ) {
                  fc_dlog( _log, "Produce priv_block timer running at ${time}", ("time", fc::time_point::now()) );
                  // pending_block_state expected, but can't assert inside async_wait
                  auto block_num = chain.pending_block_state() ? chain.pending_block_state()->block_num : 0;
                  auto res = self->maybe_produce_priv_block();
                  fc_dlog( _log, "Producing priv_Block #${num} returned: ${res}", ("num", block_num)( "res", res ) );
               }
            } ) );
   } else if (_pending_priv_block_mode == pending_block_mode::speculating && !_producers.empty() && !production_disabled_by_priv_policy()){
...
void producer_plugin_impl::schedule_delayed_production_priv_loop(const std::weak_ptr<producer_plugin_impl>& weak_this, const block_timestamp_type& current_block_time) {
   // if we have any producers then we should at least set a timer for our next available slot
   optional<fc::time_point> wake_up_time;
   for (const auto&p: _producers) {
      auto next_producer_block_time = calculate_next_priv_block_time(p, current_block_time);
...
bool producer_plugin_impl::maybe_produce_priv_block() {
   auto reschedule = fc::make_scoped_exit([this]{
      schedule_production_priv_loop();
   });
   ...
   try {
      try {
         ...
         priv_chain_plug->handle_guard_exception(e);
         return false;
      } FC_LOG_AND_DROP();
   } catch ( boost::interprocess::bad_alloc&) {
      raise(SIGUSR1);
   ...
   fc_dlog(_log, "Aborting block due to produce_priv_block error");
   chain::priv_controller& chain = priv_chain_plug->chain();
   chain.abort_block();
   return false;
}
...
void producer_plugin_impl::produce_block() {
   ...
   const auto& pbs = chain.pending_block_state();
   // for (auto a : pbs->trxs)
   //    std::cout << "wook : producer_block() : pbs->trxs : " << fc::json::to_pretty_string(a->packed_trx->get_signed_transaction()) << std::endl;
   const auto& hbs = chain.head_block_state();
...
void producer_plugin_impl::produce_priv_block() {
   //ilog("produce_block ${t}", ("t", fc::time_point::now())); // for testing _produce_time_offset_us
   EOS_ASSERT(_pending_priv_block_mode == pending_block_mode::producing, producer_exception, "called produce_block while not actually producing");
   chain::priv_controller& chain = priv_chain_plug->chain();
   const auto& pbs = chain.pending_block_state();
   //for(auto a : pbs->trxs)
   const auto& hbs = chain.head_block_state();
   ...
   auto hbt = chain.head_block_time();
   //idump((fc::time_point::now() - hbt));
   priv_block_state_ptr new_bs = chain.head_block_state();

   _producer_watermarks[new_bs->header.producer] = chain.head_block_num();
   ilog("Produced priv_block ${id}... #${n} @ ${t} signed by ${p} [trxs: ${count}, lib: ${lib}, confirmed: ${confs}]",
        ("p",new_bs->header.producer)("id",fc::variant(new_bs->id).as_string().substr(0,16))
        ("n",new_bs->block_num)("t",new_bs->header.timestamp)
        ("count",new_bs->block->transactions.size())("lib",chain.last_irreversible_block_num())("confs", new_bs->header.confirmed));
}
